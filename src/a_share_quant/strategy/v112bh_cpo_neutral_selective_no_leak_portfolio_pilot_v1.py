...
class V112BHCpoNeutralSelectiveNoLeakPortfolioPilotReport:
...
class V112BHCpoNeutralSelectiveNoLeakPortfolioPilotAnalyzer:
    HOLDING_DAYS = 20
    ...
    def analyze(
        ...
        *,
    ...
    ) -> V112BHCpoNeutralSelectiveNoLeakPortfolioPilotReport:
        if not bool(dict(phase_charter_payload.get("summary", {})).get("do_open_v112bh_now")):
            raise ValueError("V1.12BH must be open before the neutral selective pilot runs.")
        if int(dict(v112bc_protocol_payload.get("summary", {})).get("no_leak_track_count", 0)) != 2:
            raise ValueError("V1.12BH requires the frozen no-leak tracks from V1.12BC.")

        recommendations = {str(row.get("recommendation_name")): row for row in list(gap_review_payload.get("recommendation_rows", []))}
        ...
        if len(training_layer_rows) != 10:
            raise ValueError("V1.12BH expects the frozen 10-row training-facing layer.")

        margin_floor = float(recommendations["minimum_probability_margin_floor"]["recommended_value"])
        ...
        while idx < len(ordered_dates):
            ...
            if not self._passes_selective_gates(
                row=chosen_row,
                margin_floor=margin_floor,
                confidence_floor=confidence_floor,
                rollforward_floor=rollforward_floor,
                turnover_cap=turnover_cap,
                breadth_floor=breadth_floor,
                catalyst_floor=catalyst_floor,
            ):
                self._append_equity_point(equity_curve_rows, trade_date, current_equity, "cash", "CASH")
                idx += 1
                continue
            ...
            trade_rows.append(
                {
                    ...
                    "predicted_winner_prob": round(float(chosen_row["winner_prob"]), 4),
                    "probability_margin": round(float(chosen_row["probability_margin"]), 4),
                    "selective_score": round(float(chosen_row["selective_score"]), 4),
        ...
        comparison_rows = [
            {
                ...
                "neutral_value": round(total_return, 4),
                "delta": round(total_return - float(aggressive_summary.get("total_return", 0.0)), 4),
            },
            {
                "comparison_name": "max_drawdown_vs_aggressive",
                "aggressive_value": aggressive_summary.get("max_drawdown"),
                "neutral_value": round(float(max_drawdown), 4),
                "delta": round(float(max_drawdown) - float(aggressive_summary.get("max_drawdown", 0.0)), 4),
            },
            {
                "comparison_name": "profit_factor_vs_aggressive",
                "aggressive_value": aggressive_summary.get("profit_factor"),
                "neutral_value": round(float(profit_factor), 4) if profit_factor != math.inf else "inf",
                "delta": (
                    round(float(profit_factor) - float(aggressive_summary.get("profit_factor", 0.0)), 4)
                    if profit_factor != math.inf and str(aggressive_summary.get("profit_factor")) != "inf"
                    else "n/a"
                ),
            },
            {
        ...
        summary = {
            "acceptance_posture": "freeze_v112bh_cpo_neutral_selective_no_leak_portfolio_pilot_v1",
            ...
            "recommended_next_posture": "compare_oracle_aggressive_neutral_and_then_decide_model_search_expansion",
        }
        interpretation = [
            "The neutral selective track accepts cash and uses a stricter gate stack than the aggressive no-leak line.",
            "It remains point-in-time and non-deployable; its role is to test whether better return-drawdown discipline can be achieved on the same lawful 10-row layer.",
            "This is still a bounded portfolio experiment rather than a formal production signal.",
        ]
        return V112BHCpoNeutralSelectiveNoLeakPortfolioPilotReport(
            summary=summary,
            ...
            plot_bundle_rows=[
                {"plot_name": "neutral_selective_equity_curve", "series_name": "equity", "point_count": len(equity_curve_rows)},
                {"plot_name": "neutral_selective_drawdown_curve", "series_name": "drawdown", "point_count": len(drawdown_curve_rows)},
            ],
            interpretation=interpretation,
        )

    def _passes_selective_gates(
        self,
        *,
        row: pd.Series,
        margin_floor: float,
        confidence_floor: float,
        rollforward_floor: float,
        turnover_cap: float,
        breadth_floor: float,
        catalyst_floor: float,
    ) -> bool:
        return (
            float(row["probability_margin"]) >= margin_floor
            and float(row["confidence_tier_numeric"]) >= confidence_floor
            and float(row["rollforward_state_numeric"]) >= rollforward_floor
            and float(row["turnover_state_numeric"]) <= turnover_cap
            and float(row["weighted_breadth_ratio"]) >= breadth_floor
            and float(row["catalyst_presence_proxy"]) >= catalyst_floor
        )
...
def write_v112bh_cpo_neutral_selective_no_leak_portfolio_pilot_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V112BHCpoNeutralSelectiveNoLeakPortfolioPilotReport,
) -> Path:
