...
@dataclass(slots=True)
class V127ACommercialAerospacePhaseAwareDeriskAuditReport:
...
class V127ACommercialAerospacePhaseAwareDeriskAuditAnalyzer:
    ...
    def _select_derisk_candidates(
        self,
        *,
        risk_candidates: list[dict[str, Any]],
        impulse_probe_threshold: float,
        mode: str,
    ) -> tuple[list[dict[str, Any]], float]:
        if not risk_candidates:
            return [], 0.0
        if mode == "broad_half_reference":
            return risk_candidates, 0.5
        if mode == "phase_aware_weak_full_exit":
            selected = [
                c
                for c in risk_candidates
                if c["eligibility_score"] > impulse_probe_threshold
                and c["current_phase"] not in {"preheat_window", "impulse_window"}
            ]
            return selected, 1.0
        ordered = sorted(
            [
                c
                for c in risk_candidates
                if c["current_phase"] not in {"preheat_window", "impulse_window"}
            ],
            key=lambda item: item["eligibility_score"],
            reverse=True,
        )
        keep_n = max(1, len(ordered) // 2)
        return ordered[:keep_n], 1.0
            ...
            if matured_cutoff_idx >= 0:
                ...
                if available_rows and eval_rows:
                    ...
                    if next_trade_date:
                        ...
                        selected_risk, sell_ratio = self._select_derisk_candidates(
                            risk_candidates=risk_candidates,
                            impulse_probe_threshold=impulse_probe_threshold,
                            mode=config.derisk_mode,
                        )
                            ...
                            pending_orders.setdefault(next_trade_date, []).append(
                                {
                                    ...
                                    "action": "sell",
                                    "reason": f"phase_aware_{config.derisk_mode}",
    ...
    def analyze(self) -> V127ACommercialAerospacePhaseAwareDeriskAuditReport:
        ...
        variants = [
            _Config("broad_half_reference", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "broad_half"),
            _Config("phase_aware_weak_full_exit", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "phase_aware_weak_full_exit"),
            _Config("phase_aware_weakest_half_full_exit", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "phase_aware_weakest_half_full_exit"),
        ]
        variant_rows = [self._simulate_variant(rows, test_dates, date_to_idx, variant) for variant in variants]
        v126v = json.loads(self.v126v_path.read_text(encoding="utf-8"))["summary"]
        ...
        summary = {
            "acceptance_posture": "freeze_v127a_commercial_aerospace_phase_aware_derisk_audit_v1",
            "reference_v126v_final_equity": v126v["final_equity"],
            "reference_v126v_max_drawdown": v126v["max_drawdown"],
            "reference_v126x_best_variant": x_best_name,
            "reference_v126x_best_final_equity": x_best_row["final_equity"],
            "reference_v126x_best_max_drawdown": x_best_row["max_drawdown"],
        ...
        interpretation = [
            "V1.27A asks a narrower downside question than V1.26X: do we improve de-risk by only cutting names that are both weak and already outside active preheat/impulse windows?",
            "This tests timing-of-sell semantics rather than simply making sells broader, stronger, or more selective by score alone.",
        ]
        return V127ACommercialAerospacePhaseAwareDeriskAuditReport(
            summary=summary,
...
def write_report(*, reports_dir: Path, report_name: str, result: V127ACommercialAerospacePhaseAwareDeriskAuditReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v127a_commercial_aerospace_phase_aware_derisk_audit_v1",
        result=result,
    )
    print(output_path)
