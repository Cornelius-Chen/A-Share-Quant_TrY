...
class _TailRepairConfig:
    name: str
    main_riskoff_sell_ratio: float
    main_weakdrift_sell_ratio: float
    main_sentiment_sell_ratio: float
    main_impulse_sell_ratio: float
    post_weakdrift_sell_ratio: float
    post_sentiment_sell_ratio: float
    post_impulse_sell_ratio: float


@dataclass(slots=True)
class V128HCommercialAerospacePostWindowTailRepairAuditReport:
...
class V128HCommercialAerospacePostWindowTailRepairAuditAnalyzer:
    ...
    def _simulate_variant(self, config: _TailRepairConfig) -> dict[str, Any]:
                    ...
                    if next_trade_date:
                        ...
                        for row, es, ds in zip(eval_rows_z, eligibility_eval_scores, de_risk_eval_scores, strict=False):
                            ...
                            if (
                                ...
                                if in_main_window:
                                    impulse_sell_ratio = config.main_impulse_sell_ratio
                                elif in_post_window:
                                    impulse_sell_ratio = config.post_impulse_sell_ratio
                                risk_candidates.append(
                                    {
                                        "symbol": symbol,
    ...
    def analyze(self) -> V128HCommercialAerospacePostWindowTailRepairAuditReport:
        configs = [
            _TailRepairConfig("mainwin_overdrive_075_impulse_075", 1.0, 0.75, 0.75, 0.75, 0.5, 0.5, 0.5),
            _TailRepairConfig("tail_weakdrift_full", 1.0, 0.75, 0.75, 0.75, 1.0, 0.75, 0.75),
            _TailRepairConfig("tail_sentiment_full", 1.0, 0.75, 0.75, 0.75, 0.75, 1.0, 0.75),
            _TailRepairConfig("tail_weakdrift_sentiment_full", 1.0, 0.75, 0.75, 0.75, 1.0, 1.0, 0.75),
            _TailRepairConfig("tail_weakdrift_sentiment_impulse_full", 1.0, 0.75, 0.75, 0.75, 1.0, 1.0, 1.0),
        ]
        ...
        summary = {
            "acceptance_posture": "freeze_v128h_commercial_aerospace_post_window_tail_repair_audit_v1",
        ...
        interpretation = [
            "V1.28H targets only the post-window tail after the main-window deeper downside branch was promoted.",
            "The audit leaves the winning main-window grammar intact and asks whether stronger weak-drift/sentiment/impulse selling after 20260212 can reduce the residual tail giveback.",
        ]
        return V128HCommercialAerospacePostWindowTailRepairAuditReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V128HCommercialAerospacePostWindowTailRepairAuditReport,
) -> Path:
...
def main() -> None:
    ...
    result = V128HCommercialAerospacePostWindowTailRepairAuditAnalyzer(repo_root).analyze()
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v128h_commercial_aerospace_post_window_tail_repair_audit_v1",
        result=result,
    )
    print(output_path)
