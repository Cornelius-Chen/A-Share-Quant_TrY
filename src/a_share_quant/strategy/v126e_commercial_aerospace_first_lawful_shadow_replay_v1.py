...
class V126ECommercialAerospaceFirstLawfulShadowReplayReport:
...
class V126ECommercialAerospaceFirstLawfulShadowReplayAnalyzer:
    ...
    def analyze(self) -> V126ECommercialAerospaceFirstLawfulShadowReplayReport:
        ...
        eligibility_scores_train = self._score_rows(
            train_rows_z,
            train_rows_z,
            {"probe_eligibility_target", "full_eligibility_target"},
        )
        de_risk_scores_train = self._score_rows(
            train_rows_z,
            train_rows_z,
            {"de_risk_target"},
        )
        eligibility_scores_test = self._score_rows(
            train_rows_z,
            test_rows_z,
            {"probe_eligibility_target", "full_eligibility_target"},
        )
        de_risk_scores_test = self._score_rows(
            train_rows_z,
            test_rows_z,
            {"de_risk_target"},
        )

        eligibility_threshold = _quantile(eligibility_scores_train, 0.8)
        de_risk_threshold = _quantile(de_risk_scores_train, 0.8)

        scored_rows: list[dict[str, Any]] = []
        for row, eligibility_score, de_risk_score in zip(test_rows_z, eligibility_scores_test, de_risk_scores_test, strict=False):
            enriched = dict(row)
            enriched["eligibility_score"] = eligibility_score
            enriched["de_risk_score"] = de_risk_score
        ...
        for trade_date in test_dates:
            ...
            for order in todays_orders:
                ...
                if order["action"] == "buy":
                    ...
                    total_cash_needed = notional + costs["total_cost"]
            ...
            for row in by_date.get(trade_date, []):
                ...
                if not next_trade_date:
                    continue
                if (
                    row["eligibility_score"] <= eligibility_threshold
                    and row["regime_proxy_semantic"] == "impulse_expansion_proxy"
                    ...
                    pending_orders.setdefault(next_trade_date, []).append(
                        {
                            "symbol": row["symbol"],
                            "action": "buy",
                            "reason": "eligibility_binary_rc_low_pass_q80_impulse_continuation",
                        }
                    )
                if (
                    row["de_risk_score"] <= de_risk_threshold
                    and row["regime_proxy_semantic"] in {"risk_off_deterioration_proxy", "sentiment_overdrive_transition_proxy"}
                    ...
                    pending_orders.setdefault(next_trade_date, []).append(
                        {
                            "symbol": row["symbol"],
                            "action": "sell",
                            "reason": "de_risk_binary_rc_low_pass_q80_risk_regime",
                            "sell_ratio": 0.5,
                        }
                    )
        ...
        final_equity = daily_rows[-1]["equity"] if daily_rows else 1_000_000.0
        max_drawdown = max((row["drawdown"] for row in daily_rows), default=0.0)
        summary = {
            "acceptance_posture": "freeze_v126e_commercial_aerospace_first_lawful_shadow_replay_v1",
            "initial_capital": 1_000_000.0,
            "eligibility_threshold_quantile": 0.8,
            "eligibility_direction": "low_pass",
            "de_risk_threshold_quantile": 0.8,
            "de_risk_direction": "low_pass",
            "test_date_count": len(test_dates),
            "executed_order_count": len(order_rows),
            "final_equity": round(final_equity, 4),
            "max_drawdown": round(max_drawdown, 8),
            "authoritative_status": "first_lawful_narrow_eod_shadow_replay",
        }
        interpretation = [
            "V1.26E is the first lawful commercial-aerospace shadow replay under the narrowest approved boundary: EOD only, regime-conditioned, corrected low-pass score direction, and T+1 open execution.",
            "This is still a shadow protocol and not a promoted production replay stack.",
        ]
        return V126ECommercialAerospaceFirstLawfulShadowReplayReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V126ECommercialAerospaceFirstLawfulShadowReplayReport,
) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v126e_commercial_aerospace_first_lawful_shadow_replay_v1",
        result=result,
    )
    print(output_path)
