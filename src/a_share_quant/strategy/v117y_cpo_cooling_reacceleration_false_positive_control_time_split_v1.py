...
class V117YCpoCoolingReaccelerationFalsePositiveControlTimeSplitReport:
...
class V117YCpoCoolingReaccelerationFalsePositiveControlTimeSplitAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v117w_payload: dict[str, Any]) -> V117YCpoCoolingReaccelerationFalsePositiveControlTimeSplitReport:
        ...
        summary = {
            "acceptance_posture": "freeze_v117y_cpo_cooling_reacceleration_false_positive_control_time_split_v1",
            ...
            "recommended_next_posture": "send_WXY_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.17Y does not ask whether the control layer improved one full-sample number. It asks whether the control survives crude time-split transfer.",
            "A control idea that only works in-sample is still a false-positive story, not a usable candidate refinement.",
            "This closes the one extra audit cycle allowed by V117V.",
        ]
        return V117YCpoCoolingReaccelerationFalsePositiveControlTimeSplitReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V117YCpoCoolingReaccelerationFalsePositiveControlTimeSplitReport,
) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v117y_cpo_cooling_reacceleration_false_positive_control_time_split_v1",
        result=result,
    )
    print(output_path)
