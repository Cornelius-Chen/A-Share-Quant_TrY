...
class _LatePreheatFilterConfig:
    name: str
...
@dataclass(slots=True)
class V129GCommercialAerospaceLatePreheatFullFilterAuditReport:
...
class V129GCommercialAerospaceLatePreheatFullFilterAuditAnalyzer:
    ...
    def _build_full_pre_filter(self, rows: list[dict[str, Any]]) -> tuple[dict[tuple[str, str], float], dict[float, float]]:
        train_rows = [dict(row) for row in rows if row["trade_date"] <= FULL_PRE_TRAIN_END]
        eval_rows = [
            dict(row)
            for row in rows
            if LATE_PREHEAT_START <= row["trade_date"] <= LATE_PREHEAT_END
            and row["phase_window_semantic"] == "preheat_window"
        ]
        ...
        train_rows_z = transform(train_rows)
        eval_rows_z = transform(eval_rows)
        positives = [row for row in train_rows_z if row["supervised_action_state"] == "full_pre"]
        negatives = [row for row in train_rows_z if row["supervised_action_state"] != "full_pre"]
        ...
        pos_cat = {feature: Counter(row[feature] for row in positives).most_common(1)[0][0] for feature in CATEGORICAL_FEATURES}
        neg_cat = {feature: Counter(row[feature] for row in negatives).most_common(1)[0][0] for feature in CATEGORICAL_FEATURES}

        def score(row: dict[str, Any]) -> float:
            pos_dist = sum((float(row[f"{feature}_z"]) - float(pos_numeric[feature])) ** 2 for feature in NUMERIC_FEATURES)
            neg_dist = sum((float(row[f"{feature}_z"]) - float(neg_numeric[feature])) ** 2 for feature in NUMERIC_FEATURES)
            pos_penalty = sum(0.0 if row[feature] == pos_cat[feature] else 0.5 for feature in CATEGORICAL_FEATURES)
            neg_penalty = sum(0.0 if row[feature] == neg_cat[feature] else 0.5 for feature in CATEGORICAL_FEATURES)
            return (neg_dist + neg_penalty) - (pos_dist + pos_penalty)

        train_scores = [score(row) for row in train_rows_z]
        score_map = {(row["trade_date"], row["symbol"]): score(row) for row in eval_rows_z}
        threshold_map = {q: _quantile(train_scores, q) for q in [0.80, 0.85, 0.90]}
        return score_map, threshold_map

    def _simulate_variant(self, config: _LatePreheatFilterConfig, *, score_map: dict[tuple[str, str], float], threshold_map: dict[float, float]) -> dict[str, Any]:
        ...
        for trade_date in test_dates:
            ...
            if matured_cutoff_idx >= 0:
                ...
                if available_rows and eval_rows:
                    ...
                    preheat_full_scores = [score for row, score in zip(train_rows_z, eligibility_train_scores, strict=False) if row["phase_window_semantic"] == "preheat_window" and row["supervised_action_label_pg"] == "full_eligibility_target"]
                    preheat_probe_scores = [score for row, score in zip(train_rows_z, eligibility_train_scores, strict=False) if row["phase_window_semantic"] == "preheat_window" and row["supervised_action_label_pg"] in {"probe_eligibility_target", "full_eligibility_target"}]
                    impulse_full_scores = [score for row, score in zip(train_rows_z, eligibility_train_scores, strict=False) if row["phase_window_semantic"] == "impulse_window" and row["supervised_action_label_pg"] == "full_eligibility_target"]
                    impulse_probe_scores = [score for row, score in zip(train_rows_z, eligibility_train_scores, strict=False) if row["phase_window_semantic"] == "impulse_window" and row["supervised_action_label_pg"] in {"probe_eligibility_target", "full_eligibility_target"}]
                    risk_scores = [score for row, score in zip(train_rows_z, de_risk_train_scores, strict=False) if row["regime_proxy_semantic"] in {"risk_off_deterioration_proxy", "sentiment_overdrive_transition_proxy", "weak_drift_chop_proxy"}]
                    impulse_derisk_scores = [score for row, score in zip(train_rows_z, de_risk_train_scores, strict=False) if row["phase_window_semantic"] == "impulse_window" and row["supervised_action_label_pg"] == "de_risk_target"]
                    ...
                    if next_trade_date:
                        ...
                        for row, es, ds in zip(eval_rows_z, eligibility_eval_scores, de_risk_eval_scores, strict=False):
                            ...
                            if row["regime_proxy_semantic"] in {"risk_off_deterioration_proxy", "sentiment_overdrive_transition_proxy", "weak_drift_chop_proxy"} and ds <= de_risk_threshold and symbol in positions and all(p["symbol"] != symbol or p["action"] != "sell" for p in pending_orders.get(next_trade_date, [])):
                                sell_ratio = 0.5
                                if in_main_window and row["regime_proxy_semantic"] == "risk_off_deterioration_proxy":
                                    ...
                                    sell_ratio = 0.75
                                risk_candidates.append({"symbol": symbol, "sell_ratio": sell_ratio, "reason": f"tail_derisk_{row['regime_proxy_semantic']}"})
                            if row["phase_window_semantic"] == "impulse_window" and row["supervised_action_label_pg"] == "de_risk_target" and ds <= impulse_derisk_threshold and symbol in positions and all(p["symbol"] != symbol or p["action"] != "sell" for p in pending_orders.get(next_trade_date, [])):
                                impulse_sell_ratio = 0.75 if in_main_window or in_post_window else 0.5
                                risk_candidates.append({"symbol": symbol, "sell_ratio": impulse_sell_ratio, "reason": "tail_derisk_impulse_target"})
                            ...
                            if row["phase_window_semantic"] == "preheat_window":
                                if preheat_full_scores and es <= preheat_full_threshold:
                                    ...
                                    if config.filter_quantile is not None and LATE_PREHEAT_START <= trade_date <= LATE_PREHEAT_END:
                                        dedicated_score = score_map.get((trade_date, symbol), float("-inf"))
                                        threshold = threshold_map[config.filter_quantile]
                                        if dedicated_score < threshold:
                                            late_preheat_filter_hits += 1
                                            if config.fallback_mode == "block":
                                                continue
                                            if config.fallback_mode == "downgrade":
                                                target_notional = 40_000.0
                                                reason = "phase_geometry_preheat_full_downgraded_to_probe"
                                    preheat_candidates.append({"symbol": symbol, "score": es, "target_notional": target_notional, "reason": reason})
                        ...
                        for candidate in merged_sells.values():
                            pending_orders.setdefault(next_trade_date, []).append({"symbol": candidate["symbol"], "action": "sell", "reason": candidate["reason"], "sell_ratio": candidate["sell_ratio"]})
            ...
            daily_rows.append({"trade_date": trade_date, "cash": round(cash, 4), "market_value": round(market_value, 4), "equity": round(equity, 4), "drawdown": round(drawdown, 8)})
        ...
        return {
            ...
            "summary": {
                ...
                "late_preheat_filter_hits": late_preheat_filter_hits,
            },
        }

    def analyze(self) -> V129GCommercialAerospaceLatePreheatFullFilterAuditReport:
        state_table = V128YCommercialAerospaceStateMachineSupervisedTableAnalyzer(self.repo_root).analyze()
        score_map, threshold_map = self._build_full_pre_filter(state_table.training_rows)

        configs = [
            _LatePreheatFilterConfig("tail_weakdrift_full_reference", None, "reference"),
            _LatePreheatFilterConfig("late_preheat_full_q80_downgrade", 0.80, "downgrade"),
            _LatePreheatFilterConfig("late_preheat_full_q85_downgrade", 0.85, "downgrade"),
            _LatePreheatFilterConfig("late_preheat_full_q90_downgrade", 0.90, "downgrade"),
            _LatePreheatFilterConfig("late_preheat_full_q80_block", 0.80, "block"),
            _LatePreheatFilterConfig("late_preheat_full_q85_block", 0.85, "block"),
            _LatePreheatFilterConfig("late_preheat_full_q90_block", 0.90, "block"),
        ]
        payloads = [self._simulate_variant(config, score_map=score_map, threshold_map=threshold_map) for config in configs]
        variant_rows = [
            {
                ...
                "late_preheat_filter_hits": payload["summary"]["late_preheat_filter_hits"],
            }
            for payload in payloads
        ]
        reference = variant_rows[0]
        best_variant = max(variant_rows, key=lambda row: (row["final_equity"], -row["max_drawdown"], -row["executed_order_count"]))
        summary = {
            "acceptance_posture": "freeze_v129g_commercial_aerospace_late_preheat_full_filter_audit_v1",
            ...
            "late_preheat_score_thresholds": {str(k): round(v, 8) for k, v in threshold_map.items()},
        }
        interpretation = [
            "V1.29G tests whether the newly unblocked full-pre phase-specific supervision can improve the current primary replay by filtering only late-preheat full adds.",
            "This remains intentionally narrow and lawful: it only governs 2025-12-12 to 2025-12-23 preheat-full entries, leaving the rest of the primary replay unchanged.",
        ]
        return V129GCommercialAerospaceLatePreheatFullFilterAuditReport(summary=summary, variant_rows=variant_rows, interpretation=interpretation)


def write_report(*, reports_dir: Path, report_name: str, result: V129GCommercialAerospaceLatePreheatFullFilterAuditReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v129g_commercial_aerospace_late_preheat_full_filter_audit_v1",
        result=result,
    )
    print(output_path)
