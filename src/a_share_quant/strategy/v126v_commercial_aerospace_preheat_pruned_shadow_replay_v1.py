...
@dataclass(slots=True)
class V126VCommercialAerospacePreheatPrunedShadowReplayReport:
...
class V126VCommercialAerospacePreheatPrunedShadowReplayAnalyzer:
    ...
    def _load_best_config(self) -> _Config:
    ...
    def analyze(self) -> V126VCommercialAerospacePreheatPrunedShadowReplayReport:
                        ...
                        for row, es, ds in zip(eval_rows_z, eligibility_eval_scores, de_risk_eval_scores, strict=False):
                            ...
                            if row["phase_window_semantic"] == "preheat_window":
                                if preheat_full_scores and es <= preheat_full_threshold:
                                    preheat_candidates.append({"symbol": symbol, "score": es, "target_notional": 70_000.0, "reason": "phase_geometry_preheat_full"})
        ...
        summary = {
            "acceptance_posture": "freeze_v126v_commercial_aerospace_preheat_pruned_shadow_replay_v1",
            ...
            "authoritative_status": "commercial_aerospace_preheat_pruned_shadow_replay_candidate",
        }
        interpretation = [
            "V1.26V combines the lawful preheat-full tier with the cleaner execution surface from V1.26Q.",
            "This tests whether the missing early participation can be captured without keeping the V1.26T-style drawdown blowout.",
        ]
        return V126VCommercialAerospacePreheatPrunedShadowReplayReport(
            summary=summary,
...
def write_report(*, reports_dir: Path, report_name: str, result: V126VCommercialAerospacePreheatPrunedShadowReplayReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v126v_commercial_aerospace_preheat_pruned_shadow_replay_v1",
        result=result,
    )
    print(output_path)
