...
class V123YCpoHeatConditionedRiskoffExecutionAuditReport:
...
class V123YCpoHeatConditionedRiskoffExecutionAuditAnalyzer:
    ...
    def _load_riskoff_signals(
        ...
        *,
        ...
        thresholds = {
            ...
            "q95": round(_quantile(score_values, 0.95), 6),
        }
    ...
    def analyze(self) -> V123YCpoHeatConditionedRiskoffExecutionAuditReport:
        ...
        variant_defs = [
            ...
            {
                "variant_name": "balanced_hc_riskoff_q85_g55_s20_r25",
                "symbol_cap_ratio": 0.30,
                "gross_cap_ratio": 0.70,
                "cash_floor_ratio": 0.25,
            ...
            {
                "variant_name": "balanced_hc_riskoff_q90_g55_s20_r25",
                "symbol_cap_ratio": 0.30,
                "gross_cap_ratio": 0.70,
                "cash_floor_ratio": 0.25,
            ...
            {
                "variant_name": "balanced_hc_riskoff_q85_g60_s20_r25",
                "symbol_cap_ratio": 0.30,
                "gross_cap_ratio": 0.70,
                "cash_floor_ratio": 0.25,
                "riskoff_quantile": "q85",
            ...
            {
                "variant_name": "balanced_hc_riskoff_q85_g55_s25_r25",
                "symbol_cap_ratio": 0.30,
                "gross_cap_ratio": 0.70,
                "cash_floor_ratio": 0.25,
                "riskoff_quantile": "q85",
            ...
            {
                "variant_name": "balanced_hc_riskoff_q85_g55_s20_r15",
                "symbol_cap_ratio": 0.30,
                "gross_cap_ratio": 0.70,
                "cash_floor_ratio": 0.25,
                "riskoff_quantile": "q85",
                "reduce_fraction": 0.15,
                "heat_gross_trigger": 0.55,
                "heat_symbol_trigger": 0.20,
            },
        ]
        ...
        for variant in variant_defs:
            ...
            for trade_date in trade_dates:
                ...
                if variant["riskoff_quantile"] is not None:
                    threshold = riskoff_thresholds[str(variant["riskoff_quantile"])]
                    for row in riskoff_signals_by_exec.get(trade_date_str, []):
                        symbol = str(row["symbol"])
                        ...
                        if any(
                            ...
                            continue
                        if _to_float(row["score"]) < threshold:
                            continue
                        symbol_ratio_open = open_symbol_values.get(symbol, 0.0) / equity_open
                        if gross_ratio_open < float(variant["heat_gross_trigger"]):
                            suppressed_reduce_count += 1
                            continue
                        if symbol_ratio_open < float(variant["heat_symbol_trigger"]):
                            suppressed_reduce_count += 1
                            continue
                        bar = daily_bars.get((symbol, trade_date))
                        if bar is None:
                        ...
                        reduce_qty = int((held_qty * float(variant["reduce_fraction"])) // 100) * 100
                        if reduce_qty <= 0:
                            continue
                        notional = reduce_qty * execution_price
                        costs = _costs(action="sell", notional=notional)
                        cash += notional - _to_float(costs["total_cost"])
                        positions[symbol] = max(0, held_qty - reduce_qty)
                        updated_symbol_value = positions[symbol] * execution_price
                        open_symbol_values[symbol] = updated_symbol_value
                        reduced_symbols.add(symbol)
                        executed_reduce_count += 1
            ...
            variant_rows.append(
                {
                    ...
                    "reduce_fraction": variant["reduce_fraction"],
                    "executed_reduce_count": executed_reduce_count,
                    "suppressed_reduce_count": suppressed_reduce_count,
        ...
        summary = {
            "acceptance_posture": "freeze_v123y_cpo_heat_conditioned_riskoff_execution_audit_v1",
            ...
            "recommended_next_posture": "compare_heat_conditioned_riskoff_to_top_drawdown_intervals_before_any_triage",
        }
        interpretation = [
            "V1.23Y retries risk-off execution only inside elevated-heat states instead of firing broad de-risking everywhere.",
            "The aim is to preserve the heat guardrail's return profile while using risk-off only when the book is already too full.",
            "This is still research-only and must beat the plain balanced heat reference before any policy upgrade is considered.",
        ]
        return V123YCpoHeatConditionedRiskoffExecutionAuditReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V123YCpoHeatConditionedRiskoffExecutionAuditReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V123YCpoHeatConditionedRiskoffExecutionAuditAnalyzer(repo_root=repo_root)
    result = analyzer.analyze()
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v123y_cpo_heat_conditioned_riskoff_execution_audit_v1",
        result=result,
    )
    print(output_path)
