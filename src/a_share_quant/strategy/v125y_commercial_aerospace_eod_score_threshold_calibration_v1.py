...
CALIBRATION_QUANTILES = [0.6, 0.7, 0.8, 0.9]


@dataclass(slots=True)
class V125YCommercialAerospaceEODScoreThresholdCalibrationReport:
...
class V125YCommercialAerospaceEODScoreThresholdCalibrationAnalyzer:
    ...
    def analyze(self) -> V125YCommercialAerospaceEODScoreThresholdCalibrationReport:
        ...
        for task_name, config in TASKS.items():
            scores_train = self._score_rows(train_rows_z, train_rows_z, config["positive"])
            scores_test = self._score_rows(train_rows_z, test_rows_z, config["positive"])
            for q in CALIBRATION_QUANTILES:
                threshold = _quantile(scores_train, q)
                passed = [row for row, score in zip(test_rows_z, scores_test, strict=False) if score >= threshold]
                failed = [row for row, score in zip(test_rows_z, scores_test, strict=False) if score < threshold]
                if task_name == "eligibility_binary":
                    ...
                    spread = pass_mean - fail_mean
                else:
                    pass_mean = _mean([float(row["forward_return_10"]) for row in passed])
                    fail_mean = _mean([float(row["forward_return_10"]) for row in failed])
                    spread = fail_mean - pass_mean
        ...
        for row in threshold_rows:
            ...
            if row["test_spread"] > best_rows[row["task_name"]]["test_spread"]:
                best_rows[row["task_name"]] = row

        summary = {
            "acceptance_posture": "freeze_v125y_commercial_aerospace_eod_score_threshold_calibration_v1",
            "train_row_count": len(train_rows_z),
            "test_row_count": len(test_rows_z),
            "best_eligibility_spread": best_rows["eligibility_binary"]["test_spread"],
            "best_de_risk_spread": best_rows["de_risk_binary"]["test_spread"],
            "authoritative_status": "lawful_eod_score_calibration_non_execution",
        }
        interpretation = [
            "V1.25Y calibrates score thresholds on train-only quantiles and then asks whether test-side forward spread is economically meaningful.",
            "This is the bridge from lawful offline learning into replay-adjacent score semantics.",
        ]
        return V125YCommercialAerospaceEODScoreThresholdCalibrationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V125YCommercialAerospaceEODScoreThresholdCalibrationReport,
) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v125y_commercial_aerospace_eod_score_threshold_calibration_v1",
        result=result,
    )
    print(output_path)
