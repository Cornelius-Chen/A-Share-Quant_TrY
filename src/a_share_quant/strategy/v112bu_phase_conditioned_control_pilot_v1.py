from __future__ import annotations

import json
import math
from bisect import bisect_right
from dataclasses import dataclass
from pathlib import Path
from typing import Any

import pandas as pd

from a_share_quant.strategy.v112a_price_cycle_inference_v1 import TencentKlineClient


@dataclass(slots=True)
class V112BUPhaseConditionedControlPilotReport:
    summary: dict[str, Any]
    trade_rows: list[dict[str, Any]]
    equity_curve_rows: list[dict[str, Any]]
    drawdown_curve_rows: list[dict[str, Any]]
    control_action_rows: list[dict[str, Any]]
    comparison_rows: list[dict[str, Any]]
    interpretation: list[str]

    def as_dict(self) -> dict[str, Any]:
        return {
            "summary": self.summary,
            "trade_rows": self.trade_rows,
            "equity_curve_rows": self.equity_curve_rows,
            "drawdown_curve_rows": self.drawdown_curve_rows,
            "control_action_rows": self.control_action_rows,
            "comparison_rows": self.comparison_rows,
            "interpretation": self.interpretation,
        }


def load_json_report(path: Path) -> dict[str, Any]:
    with path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    if not isinstance(payload, dict):
        raise ValueError(f"Report at {path} must decode to a mapping.")
    return payload


class V112BUPhaseConditionedControlPilotAnalyzer:
    DEFAULT_HOLDING_DAYS = 20
    HOLDING_VETO_DAYS = 10

    def analyze(
        self,
        *,
        phase_charter_payload: dict[str, Any],
        bt_extraction_payload: dict[str, Any],
        bs_refinement_payload: dict[str, Any],
        bp_fusion_payload: dict[str, Any],
        neutral_pilot_payload: dict[str, Any],
    ) -> V112BUPhaseConditionedControlPilotReport:
        if not bool(dict(phase_charter_payload.get("summary", {})).get("do_open_v112bu_now")):
            raise ValueError("V1.12BU must be open before the control pilot runs.")

        base_trade_rows = sorted(list(bp_fusion_payload.get("trade_rows", [])), key=lambda row: str(row.get("entry_date")))
        base_equity_rows = list(bp_fusion_payload.get("equity_curve_rows", []))
        if not base_trade_rows or not base_equity_rows:
            raise ValueError("V1.12BU requires the frozen fusion baseline artifacts.")

        entry_veto_pairs = {
            (str(row.get("stage_family")), str(row.get("role_family")))
            for row in list(bt_extraction_payload.get("entry_veto_rows", []))
        }
        holding_veto_pairs = {
            (str(row.get("stage_family")), str(row.get("role_family")))
            for row in list(bt_extraction_payload.get("holding_veto_rows", []))
        }
        risk_dates = {
            str(row.get("trade_date"))
            for row in list(bs_refinement_payload.get("veto_neighborhood_rows", []))
        } | {
            str(row.get("trade_date"))
            for row in list(bs_refinement_payload.get("transition_band_rows", []))
        }

        symbols = sorted({str(row.get("symbol")) for row in base_trade_rows})
        bar_cache = self._bar_cache(symbols=symbols)
        ordered_dates = [str(row.get("trade_date")) for row in base_equity_rows]
        equity_curve_rows: list[dict[str, Any]] = []
        trade_rows: list[dict[str, Any]] = []
        control_action_rows: list[dict[str, Any]] = []
        current_equity = 1.0
        current_idx = 0

        for trade in base_trade_rows:
            entry_date = str(trade.get("entry_date"))
            stage_family = str(trade.get("stage_family"))
            role_family = str(trade.get("role_family"))
            symbol = str(trade.get("symbol"))

            entry_idx = bisect_right(ordered_dates, entry_date) - 1
            if entry_idx < 0:
                continue
            while current_idx <= entry_idx and current_idx < len(ordered_dates):
                self._append_equity_point(equity_curve_rows, ordered_dates[current_idx], current_equity, "cash", "CASH")
                current_idx += 1

            veto_reason = None
            if (stage_family, role_family) in entry_veto_pairs:
                veto_reason = "entry_veto"
            elif entry_date in risk_dates:
                veto_reason = "risk_off_override"

            if veto_reason is not None:
                control_action_rows.append(
                    {
                        "entry_date": entry_date,
                        "symbol": symbol,
                        "stage_family": stage_family,
                        "role_family": role_family,
                        "control_action": veto_reason,
                    }
                )
                continue

            holding_days = self.HOLDING_VETO_DAYS if (stage_family, role_family) in holding_veto_pairs else self.DEFAULT_HOLDING_DAYS
            if holding_days != self.DEFAULT_HOLDING_DAYS:
                control_action_rows.append(
                    {
                        "entry_date": entry_date,
                        "symbol": symbol,
                        "stage_family": stage_family,
                        "role_family": role_family,
                        "control_action": "holding_veto_half_life",
                    }
                )

            trade_path = self._trade_path(
                symbol=symbol,
                entry_date=entry_date,
                bars=bar_cache[symbol],
                holding_days=holding_days,
            )
            if not trade_path:
                continue

            entry_equity = current_equity
            peak_equity = entry_equity
            max_path_drawdown = 0.0
            for point in trade_path:
                path_equity = entry_equity * float(point["price_ratio"])
                peak_equity = max(peak_equity, path_equity)
                if peak_equity > 0.0:
                    max_path_drawdown = min(max_path_drawdown, path_equity / peak_equity - 1.0)
                self._append_equity_point(equity_curve_rows, str(point["trade_date"]), path_equity, "long", symbol)

            current_equity = float(equity_curve_rows[-1]["equity"])
            exit_date = str(trade_path[-1]["trade_date"])
            trade_rows.append(
                {
                    "entry_date": entry_date,
                    "exit_date": exit_date,
                    "symbol": symbol,
                    "stage_family": stage_family,
                    "role_family": role_family,
                    "holding_days": holding_days,
                    "ranker_score": trade.get("ranker_score"),
                    "realized_forward_return": round(current_equity / entry_equity - 1.0, 4),
                    "path_max_drawdown": round(float(max_path_drawdown), 4),
                    "entry_equity": round(entry_equity, 4),
                    "exit_equity": round(current_equity, 4),
                }
            )
            current_idx = bisect_right(ordered_dates, exit_date)

        while current_idx < len(ordered_dates):
            self._append_equity_point(equity_curve_rows, ordered_dates[current_idx], current_equity, "cash", "CASH")
            current_idx += 1

        drawdown_curve_rows = self._drawdown_curve(equity_curve_rows)
        total_return = float(equity_curve_rows[-1]["equity"]) - 1.0 if equity_curve_rows else 0.0
        max_drawdown = min((float(row["drawdown"]) for row in drawdown_curve_rows), default=0.0)
        positive_sum = sum(float(row["realized_forward_return"]) for row in trade_rows if float(row["realized_forward_return"]) > 0.0)
        negative_sum = sum(float(row["realized_forward_return"]) for row in trade_rows if float(row["realized_forward_return"]) < 0.0)
        profit_factor = positive_sum / abs(negative_sum) if negative_sum < 0.0 else math.inf

        bp_summary = dict(bp_fusion_payload.get("summary", {}))
        neutral_summary = dict(neutral_pilot_payload.get("summary", {}))
        base_trade_count = int(bp_summary.get("trade_count", 0))
        vetoed_trade_count = sum(1 for row in control_action_rows if str(row["control_action"]) in {"entry_veto", "risk_off_override"})
        half_life_count = sum(1 for row in control_action_rows if str(row["control_action"]) == "holding_veto_half_life")

        comparison_rows = [
            {
                "comparison_name": "return_vs_bp_fusion",
                "baseline_value": bp_summary.get("total_return"),
                "control_value": round(total_return, 4),
                "delta": round(total_return - float(bp_summary.get("total_return", 0.0)), 4),
            },
            {
                "comparison_name": "max_drawdown_vs_bp_fusion",
                "baseline_value": bp_summary.get("max_drawdown"),
                "control_value": round(float(max_drawdown), 4),
                "delta": round(float(max_drawdown) - float(bp_summary.get("max_drawdown", 0.0)), 4),
            },
            {
                "comparison_name": "return_vs_neutral",
                "baseline_value": neutral_summary.get("total_return"),
                "control_value": round(total_return, 4),
                "delta": round(total_return - float(neutral_summary.get("total_return", 0.0)), 4),
            },
            {
                "comparison_name": "max_drawdown_vs_neutral",
                "baseline_value": neutral_summary.get("max_drawdown"),
                "control_value": round(float(max_drawdown), 4),
                "delta": round(float(max_drawdown) - float(neutral_summary.get("max_drawdown", 0.0)), 4),
            },
        ]

        summary = {
            "acceptance_posture": "freeze_v112bu_phase_conditioned_control_pilot_v1",
            "trade_count": len(trade_rows),
            "vetoed_trade_count": vetoed_trade_count,
            "holding_veto_half_life_count": half_life_count,
            "base_trade_count": base_trade_count,
            "total_return": round(total_return, 4),
            "max_drawdown": round(float(max_drawdown), 4),
            "profit_factor": round(float(profit_factor), 4) if profit_factor != math.inf else "inf",
            "cash_ratio": round(
                sum(1 for row in equity_curve_rows if str(row["position_state"]) == "cash") / max(len(equity_curve_rows), 1),
                4,
            ),
            "bp_return_delta": round(total_return - float(bp_summary.get("total_return", 0.0)), 4),
            "bp_drawdown_delta": round(float(max_drawdown) - float(bp_summary.get("max_drawdown", 0.0)), 4),
            "neutral_return_delta": round(total_return - float(neutral_summary.get("total_return", 0.0)), 4),
            "neutral_drawdown_delta": round(float(max_drawdown) - float(neutral_summary.get("max_drawdown", 0.0)), 4),
            "formal_training_now": False,
            "formal_signal_generation_now": False,
            "ready_for_phase_check_next": True,
            "recommended_next_posture": "keep_phase_conditioned_controls_as_candidate_only" if total_return <= float(bp_summary.get("total_return", 0.0)) else "continue_refining_phase_conditioned_controls",
        }
        interpretation = [
            "V1.12BU is a report-only control pilot layered on top of the frozen selector-maturity fusion baseline.",
            "It tests whether explicit entry veto, holding veto, and risk-off override objects improve discipline without retraining the selector.",
        ]
        return V112BUPhaseConditionedControlPilotReport(
            summary=summary,
            trade_rows=trade_rows,
            equity_curve_rows=equity_curve_rows,
            drawdown_curve_rows=drawdown_curve_rows,
            control_action_rows=control_action_rows,
            comparison_rows=comparison_rows,
            interpretation=interpretation,
        )

    def _bar_cache(self, *, symbols: list[str]) -> dict[str, Any]:
        client = TencentKlineClient()
        cache = {}
        for symbol in symbols:
            frame = client.fetch_daily_bars(symbol).sort_values("date").reset_index(drop=True).copy()
            frame["trade_date"] = pd.to_datetime(frame["date"]).dt.strftime("%Y-%m-%d")
            cache[symbol] = frame
        return cache

    def _trade_path(self, *, symbol: str, entry_date: str, bars: Any, holding_days: int) -> list[dict[str, Any]]:
        matching = bars.index[bars["trade_date"] == entry_date].tolist()
        if not matching:
            return []
        start_idx = int(matching[0])
        end_idx = start_idx + holding_days
        if end_idx >= len(bars):
            return []
        entry_close = float(bars.iloc[start_idx]["close"])
        if entry_close == 0.0:
            return []
        rows = []
        for idx in range(start_idx, end_idx + 1):
            close = float(bars.iloc[idx]["close"])
            rows.append({"trade_date": str(bars.iloc[idx]["trade_date"]), "price_ratio": close / entry_close})
        return rows

    def _append_equity_point(
        self,
        equity_curve_rows: list[dict[str, Any]],
        trade_date: str,
        equity: float,
        position_state: str,
        symbol: str,
    ) -> None:
        row = {
            "trade_date": trade_date,
            "equity": round(float(equity), 6),
            "position_state": position_state,
            "symbol": symbol,
        }
        if equity_curve_rows and str(equity_curve_rows[-1]["trade_date"]) == trade_date:
            equity_curve_rows[-1] = row
        else:
            equity_curve_rows.append(row)

    def _drawdown_curve(self, equity_curve_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        peak = 0.0
        rows: list[dict[str, Any]] = []
        for row in equity_curve_rows:
            equity = float(row["equity"])
            peak = max(peak, equity)
            drawdown = equity / peak - 1.0 if peak > 0.0 else 0.0
            rows.append({"trade_date": str(row["trade_date"]), "drawdown": round(float(drawdown), 6)})
        return rows


def write_v112bu_phase_conditioned_control_pilot_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V112BUPhaseConditionedControlPilotReport,
) -> Path:
    reports_dir.mkdir(parents=True, exist_ok=True)
    output_path = reports_dir / f"{report_name}.json"
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(result.as_dict(), handle, indent=2, ensure_ascii=False)
    return output_path
