...
class V126CCommercialAerospaceRegimeConditionedEconomicAuditReport:
...
class V126CCommercialAerospaceRegimeConditionedEconomicAuditAnalyzer:
    ...
    def analyze(self) -> V126CCommercialAerospaceRegimeConditionedEconomicAuditReport:
        ...
        for task_name, config in TASKS.items():
            ...
            passed = [row for row, pred in zip(test_rows_z, y_pred, strict=False) if pred == 1]
            failed = [row for row, pred in zip(test_rows_z, y_pred, strict=False) if pred == 0]
        ...
        best_task = max(task_rows, key=lambda row: row["economic_spread"])
        summary = {
            "acceptance_posture": "freeze_v126c_commercial_aerospace_regime_conditioned_economic_audit_v1",
            "test_row_count": len(test_rows_z),
            "best_task_name": best_task["task_name"],
            "best_economic_spread": best_task["economic_spread"],
            "authoritative_status": "regime_conditioned_economic_audit_non_execution",
        }
        interpretation = [
            "V1.26C asks the replay-adjacent question directly: after regime-conditioned relabeling, do predicted positive actions now carry actual test-side economic spread?",
            "This is the last checkpoint before any lawful shadow replay discussion.",
        ]
        return V126CCommercialAerospaceRegimeConditionedEconomicAuditReport(
            summary=summary,
            task_rows=task_rows,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V126CCommercialAerospaceRegimeConditionedEconomicAuditReport,
) -> Path:
