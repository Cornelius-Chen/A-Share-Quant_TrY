...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("sustained_participation_non_chase_score")) >= threshold)
    fn = len(positives) - tp
    fp = sum(1 for row in negatives if _to_float(row.get("sustained_participation_non_chase_score")) >= threshold)
    tn = len(negatives) - fp
...
class V118VCpoSustainedParticipationNonChaseTimeSplitValidationReport:
...
class V118VCpoSustainedParticipationNonChaseTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v118u_payload: dict[str, Any]) -> V118VCpoSustainedParticipationNonChaseTimeSplitValidationReport:
        rows = list(v118u_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("sustained_participation_non_chase_score")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v118v_cpo_sustained_participation_non_chase_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_TUV_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.18V applies immediate chronology discipline to the sustained-participation non-chase branch.",
            "A line that cannot survive crude year splits should be stopped even if its full-sample gap is attractive.",
            "This is the project's standard safeguard against same-sample semantic overfitting.",
        ]
        return V118VCpoSustainedParticipationNonChaseTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V118VCpoSustainedParticipationNonChaseTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V118VCpoSustainedParticipationNonChaseTimeSplitValidationAnalyzer(repo_root=repo_root)
    ...
    output_path = write_report(
        ...
        report_name="v118v_cpo_sustained_participation_non_chase_time_split_validation_v1",
        result=result,
    )
    print(output_path)
