...
class V121FCpoCloseContextTimeSplitAuditReport:
...
class V121FCpoCloseContextTimeSplitAuditAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v121e_payload: dict[str, Any]) -> V121FCpoCloseContextTimeSplitAuditReport:
        ...
        summary = {
            "acceptance_posture": "freeze_v121f_cpo_close_context_time_split_audit_v1",
            ...
            "stability_posture": (
                "candidate_stable_enough_inside_close_context"
                if min_test_balanced_accuracy >= 0.5
                else "candidate_unstable_inside_close_context"
            ),
            "recommended_next_posture": "triage_EF_before_any_more_narrowing",
        }
        interpretation = [
            "V1.21F asks the only chronology question that currently matters for this downside line: does it stay alive once the action context is fixed to close_vs_hold?",
            "This is still not a replay or promotion step; it is only a narrowing audit.",
            "If it survives here, the branch may justify one more non-replay refinement cycle.",
        ]
        return V121FCpoCloseContextTimeSplitAuditReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V121FCpoCloseContextTimeSplitAuditReport,
) -> Path:
