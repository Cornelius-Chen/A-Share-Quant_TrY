...
class V119DCpoTushareTurnoverDisciplineDiscoveryReport:
...
class V119DCpoTushareTurnoverDisciplineDiscoveryAnalyzer:
    ...
    def _joined_rows(self) -> list[dict[str, Any]]:
        ...
        for row in base_rows:
            ...
            merged["large_buy_sell_ratio"] = (
                (merged["buy_lg_amount"] + 1.0) / (merged["sell_lg_amount"] + 1.0)
            )
            joined_rows.append(merged)
        return joined_rows

    def analyze(self) -> V119DCpoTushareTurnoverDisciplineDiscoveryReport:
        ...
        large_ratio_values = [_to_float(row["large_buy_sell_ratio"]) for row in rows]
        ...
        for row in rows:
            ...
            large_ratio_z = _zscore(_to_float(row["large_buy_sell_ratio"]), large_ratio_mean, large_ratio_std)
            score = -turnover_z - volume_ratio_z + 0.5 * large_ratio_z
            candidate_score_rows.append(
                {
                    ...
                    "large_buy_sell_ratio": round(_to_float(row["large_buy_sell_ratio"]), 6),
                    "turnover_discipline_score_candidate": round(score, 6),
                }
            )
        candidate_score_rows.sort(
            key=lambda row: (not bool(row["positive_add_label"]), -_to_float(row["turnover_discipline_score_candidate"]))
        )

        positive_mean = sum(
            _to_float(row["turnover_discipline_score_candidate"]) for row in candidate_score_rows if row["positive_add_label"]
        ) / len(positive_rows)
        negative_mean = sum(
            _to_float(row["turnover_discipline_score_candidate"]) for row in candidate_score_rows if not row["positive_add_label"]
        ) / len(negative_rows)

        summary = {
            "acceptance_posture": "freeze_v119d_cpo_tushare_turnover_discipline_discovery_v1",
            ...
            "recommended_next_posture": "externally_audit_tushare_turnover_discipline_and_keep_non_replay_only",
        }
        interpretation = [
            "V1.19D opens an orthogonal hardening line using Tushare daily context instead of more same-family intraday tuning.",
            "The candidate rewards non-frothy participation: lower free-float turnover, lower volume-ratio overheat, and healthier large-order buy-sell balance.",
            "This line is intentionally discovery-only and must survive broader add-pool and time-split checks before any further attention.",
        ]
        return V119DCpoTushareTurnoverDisciplineDiscoveryReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V119DCpoTushareTurnoverDisciplineDiscoveryReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V119DCpoTushareTurnoverDisciplineDiscoveryAnalyzer(repo_root=repo_root)
    result = analyzer.analyze()
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v119d_cpo_tushare_turnover_discipline_discovery_v1",
        result=result,
    )
    print(output_path)
