...
class V116CCpoVisibleOnlyIntradayFilterRebuildReport:
...
class V116CCpoVisibleOnlyIntradayFilterRebuildAnalyzer:
    ...
    def analyze(
        ...
        *,
    ...
    ) -> V116CCpoVisibleOnlyIntradayFilterRebuildReport:
        ...
        pc1_values = sorted(_to_float(row.get("pc1_score")) for row in checkpoint_rows)
        pc2_values = sorted(_to_float(row.get("pc2_score")) for row in checkpoint_rows)
        pc1_low_q = _quantile(pc1_values, 0.33)
        pc2_low_q = _quantile(pc2_values, 0.33)

        def _with_visible(row: dict[str, Any]) -> dict[str, Any]:
        ...
        enriched_rows = [_with_visible(row) for row in candidate_rows]
        filters = {
            "all_intraday_strict_visible": lambda row: True,
            "pc1_extreme_low_only": lambda row: _to_float(row.get("visible_pc1_score")) <= pc1_low_q,
            "pc2_extreme_low_only": lambda row: _to_float(row.get("visible_pc2_score")) <= pc2_low_q,
            "pc1_or_pc2_extreme_low": lambda row: _to_float(row.get("visible_pc1_score")) <= pc1_low_q or _to_float(row.get("visible_pc2_score")) <= pc2_low_q,
            "pc1_and_pc2_extreme_low": lambda row: _to_float(row.get("visible_pc1_score")) <= pc1_low_q and _to_float(row.get("visible_pc2_score")) <= pc2_low_q,
        }
        ...
        try:
            ...
            for variant_name, predicate in filters.items():
                            ...
                            overlay_order_rows.append(
                                {
                                    ...
                                    "overlay_posture": "visible_only_intraday_timing_aware_add_only",
                                }
                            )
        ...
        summary = {
            "acceptance_posture": "freeze_v116c_cpo_visible_only_intraday_filter_rebuild_v1",
            ...
            "recommended_next_posture": "carry_visible_only_intraday_variants_forward_and_disallow_future_label_based_filtering",
        }
        threshold_summary = {
            "pc1_low_q_33": round(pc1_low_q, 6),
            "pc2_low_q_33": round(pc2_low_q, 6),
        }
        interpretation = [
            "V1.16C rebuilds intraday timing-aware overlay filters using only point-in-time-visible checkpoint scores, with no future outcome labels allowed in the filter definitions.",
            "This is the first direct repair of the main issue raised by the V116B adversarial triage.",
            "The resulting variants remain candidate-only and are meant to replace post-hoc label filters when replaying intraday overlays.",
        ]
        return V116CCpoVisibleOnlyIntradayFilterRebuildReport(
            summary=summary,
...
def write_report(*, reports_dir: Path, report_name: str, result: V116CCpoVisibleOnlyIntradayFilterRebuildReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v116c_cpo_visible_only_intraday_filter_rebuild_v1",
        result=result,
    )
    print(output_path)
