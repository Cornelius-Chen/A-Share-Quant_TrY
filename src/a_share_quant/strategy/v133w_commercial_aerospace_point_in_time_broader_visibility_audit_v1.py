...
class V133WCommercialAerospacePointInTimeBroaderVisibilityAuditReport:
...
class V133WCommercialAerospacePointInTimeBroaderVisibilityAuditAnalyzer:
    def __init__(self, repo_root: Path) -> None:
        ...
        self.output_csv = (
            repo_root / "data" / "training" / "commercial_aerospace_point_in_time_broader_visibility_audit_v1.csv"
        )

    def analyze(self) -> V133WCommercialAerospacePointInTimeBroaderVisibilityAuditReport:
        ...
        for (trade_date, symbol), session in grouped.items():
            ...
            for row in session:
                ...
                if event_first_visible_ts is None or phase_first_visible_ts is None:
                    session_event_visibility_violations += 1
                else:
                    if event_first_visible_ts < session_open_ts or phase_first_visible_ts < session_open_ts:
                        session_event_visibility_violations += 1
        ...
        summary = {
            "acceptance_posture": "freeze_v133w_commercial_aerospace_point_in_time_broader_visibility_audit_v1",
            ...
            "authoritative_output": "commercial_aerospace_point_in_time_broader_visibility_audit_ready_for_direction_triage",
        }
        interpretation = [
            "V1.33W audits whether the broader 24-session phase-1 visibility surface still obeys the canonical point-in-time rules.",
            "The purpose is to widen legality, not to unlock simulation or replay.",
        ]
        return V133WCommercialAerospacePointInTimeBroaderVisibilityAuditReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V133WCommercialAerospacePointInTimeBroaderVisibilityAuditReport,
) -> Path:
