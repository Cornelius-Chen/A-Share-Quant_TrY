...
def _candidate_score(row: dict[str, Any]) -> float:
    return (
        _to_float(row.get("f30_lower_shadow_ratio_rz"))
        + _to_float(row.get("f60_lower_shadow_ratio_rz"))
        + _to_float(row.get("f30_last_bar_lower_shadow_ratio_rz"))
        + _to_float(row.get("f60_last_bar_lower_shadow_ratio_rz"))
        ...
        - _to_float(row.get("f30_last_bar_upper_shadow_ratio_rz"))
        - _to_float(row.get("f60_last_bar_upper_shadow_ratio_rz"))
...
class V118OCpoReclaimAbsorptionDiscoveryReport:
...
class V118OCpoReclaimAbsorptionDiscoveryAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, rows_path: Path) -> V118OCpoReclaimAbsorptionDiscoveryReport:
        rows = [
            dict(row)
            ...
            candidate_score_rows.append(
                {
                    ...
                    "reclaim_absorption_score": round(_candidate_score(row), 6),
                }
            )
        candidate_score_rows.sort(
            key=lambda row: (not bool(row["positive_add_label"]), -_to_float(row["reclaim_absorption_score"]))
        )

        positive_mean = sum(_to_float(row["reclaim_absorption_score"]) for row in candidate_score_rows if row["positive_add_label"]) / len(positive_rows)
        negative_mean = sum(_to_float(row["reclaim_absorption_score"]) for row in candidate_score_rows if not row["positive_add_label"]) / len(negative_rows)

        summary = {
            "acceptance_posture": "freeze_v118o_cpo_reclaim_absorption_discovery_v1",
            ...
            "recommended_next_posture": "audit_reclaim_absorption_on_broader_add_pool_and_keep_candidate_only",
        }
        interpretation = [
            "V1.18O opens a fresh quality-side discovery line that is materially different from cooling, breakout-damage, and add-vs-entry.",
            "The branch asks whether good add windows look like intraday absorption and reclaim: lower-wick support, contained upper-wick damage, and a reclaim back toward VWAP and the upper part of the range.",
            "This is discovery only. The line survives only if the broader external audit stays positive.",
        ]
        return V118OCpoReclaimAbsorptionDiscoveryReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V118OCpoReclaimAbsorptionDiscoveryReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V118OCpoReclaimAbsorptionDiscoveryAnalyzer(repo_root=repo_root)
    result = analyzer.analyze(
        rows_path=repo_root / "data" / "training" / "cpo_midfreq_high_dimensional_feature_base_table_v1.csv",
    )
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v118o_cpo_reclaim_absorption_discovery_v1",
        result=result,
    )
    print(output_path)
