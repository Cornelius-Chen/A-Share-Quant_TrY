...
class _Config:
    ...
    selective_derisk_mode: str


@dataclass(slots=True)
class V126XCommercialAerospaceConditionalDeriskAuditReport:
...
class V126XCommercialAerospaceConditionalDeriskAuditAnalyzer:
            ...
            if matured_cutoff_idx >= 0:
                ...
                if available_rows and eval_rows:
                    ...
                    if next_trade_date:
                        ...
                        if risk_candidates:
                            if config.selective_derisk_mode == "broad_half":
                                selected = risk_candidates
                                sell_ratio = 0.5
                            elif config.selective_derisk_mode == "weak_only_full":
                                selected = [c for c in risk_candidates if c["eligibility_score"] > impulse_probe_threshold]
                                sell_ratio = 1.0
                            else:  # weakest_half_full
                                ordered_risk = sorted(risk_candidates, key=lambda item: item["eligibility_score"], reverse=True)
                                keep_n = max(1, len(ordered_risk) // 2)
                                selected = ordered_risk[:keep_n]
                                sell_ratio = 1.0
                            for candidate in selected:
                                pending_orders.setdefault(next_trade_date, []).append(
                                    {"symbol": candidate["symbol"], "action": "sell", "reason": f"conditional_{config.selective_derisk_mode}", "sell_ratio": sell_ratio}
                                )
    ...
    def analyze(self) -> V126XCommercialAerospaceConditionalDeriskAuditReport:
        ...
        variants = [
            ...
            _Config("weak_only_full_exit", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "weak_only_full"),
            _Config("weakest_half_full_exit", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "weakest_half_full"),
        ]
        ...
        summary = {
            "acceptance_posture": "freeze_v126x_commercial_aerospace_conditional_derisk_audit_v1",
        ...
        interpretation = [
            "V1.26X audits whether commercial-aerospace downside is better controlled by selective de-risk on the weakest names instead of broad half-sells.",
            "No factor, label, or replay legality changes are introduced; this is a pure conditional sell grammar audit on top of V1.26V.",
        ]
        return V126XCommercialAerospaceConditionalDeriskAuditReport(summary=summary, variant_rows=variant_rows, interpretation=interpretation)


def write_report(*, reports_dir: Path, report_name: str, result: V126XCommercialAerospaceConditionalDeriskAuditReport) -> Path:
