...
class _DeeperGrammarConfig:
    name: str
    riskoff_sell_ratio: float
    weakdrift_sell_ratio: float
    sentiment_overdrive_sell_ratio: float
    include_impulse_derisk: bool
    impulse_sell_ratio: float


@dataclass(slots=True)
class V128ECommercialAerospaceMainWindowDeeperDownsideAuditReport:
...
class V128ECommercialAerospaceMainWindowDeeperDownsideAuditAnalyzer:
    ...
    def _simulate_variant(self, config: _DeeperGrammarConfig) -> dict[str, Any]:
                    ...
                    if next_trade_date:
                        ...
                        for row, es, ds in zip(eval_rows_z, eligibility_eval_scores, de_risk_eval_scores, strict=False):
                            ...
                            if (
                                ...
                                elif in_window and row["regime_proxy_semantic"] == "sentiment_overdrive_transition_proxy":
                                    sell_ratio = config.sentiment_overdrive_sell_ratio
    ...
    def analyze(self) -> V128ECommercialAerospaceMainWindowDeeperDownsideAuditReport:
        configs = [
            _DeeperGrammarConfig("window_riskoff_full_weakdrift_075_impulse_half", 1.0, 0.75, 0.5, True, 0.5),
            _DeeperGrammarConfig("mainwin_overdrive_075", 1.0, 0.75, 0.75, True, 0.5),
            _DeeperGrammarConfig("mainwin_overdrive_full", 1.0, 0.75, 1.0, True, 0.5),
            _DeeperGrammarConfig("mainwin_overdrive_075_impulse_075", 1.0, 0.75, 0.75, True, 0.75),
            _DeeperGrammarConfig("mainwin_overdrive_full_impulse_075", 1.0, 0.75, 1.0, True, 0.75),
        ]
        ...
        summary = {
            "acceptance_posture": "freeze_v128e_commercial_aerospace_main_window_deeper_downside_audit_v1",
        ...
        interpretation = [
            "V1.28E deepens downside control only inside the main drawdown window by tightening sentiment-overdrive and impulse-target sell intensity.",
            "The branch keeps the current primary entry surface unchanged and only tests whether the main window can be improved further without reopening broad symbol tuning.",
        ]
        return V128ECommercialAerospaceMainWindowDeeperDownsideAuditReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V128ECommercialAerospaceMainWindowDeeperDownsideAuditReport,
) -> Path:
...
def main() -> None:
    ...
    result = V128ECommercialAerospaceMainWindowDeeperDownsideAuditAnalyzer(repo_root).analyze()
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v128e_commercial_aerospace_main_window_deeper_downside_audit_v1",
        result=result,
    )
    print(output_path)
