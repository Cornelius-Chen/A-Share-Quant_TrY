...
class V119NCpoParticipationTurnoverElgSupportTimeSplitValidationReport:
...
class V119NCpoParticipationTurnoverElgSupportTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v119l_payload: dict[str, Any]) -> V119NCpoParticipationTurnoverElgSupportTimeSplitValidationReport:
        ...
        for split_name, holdout_years in split_definitions:
            ...
            train_thresholds = sorted(
                {_to_float(row["participation_turnover_elg_support_score"]) for row in train_rows},
                reverse=True,
            )
        ...
        summary = {
            "acceptance_posture": "freeze_v119n_cpo_participation_turnover_elg_support_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_LMN_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.19N checks whether the extra-large-flow support term actually helps chronology rather than just flattering the full pool.",
            "The critical test is whether the 2024 holdout improves without destroying the other year buckets.",
            "This closes the first three-run block for the ELG-supported combo line.",
        ]
        return V119NCpoParticipationTurnoverElgSupportTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V119NCpoParticipationTurnoverElgSupportTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V119NCpoParticipationTurnoverElgSupportTimeSplitValidationAnalyzer(repo_root=repo_root)
    ...
    output_path = write_report(
        ...
        report_name="v119n_cpo_participation_turnover_elg_support_time_split_validation_v1",
        result=result,
    )
    print(output_path)
