...
class V121CCpoParticipationCollapseCloseRiskTimeSplitValidationReport:
...
class V121CCpoParticipationCollapseCloseRiskTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v121b_payload: dict[str, Any]) -> V121CCpoParticipationCollapseCloseRiskTimeSplitValidationReport:
        rows = list(v121b_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        summary = {
            "acceptance_posture": "freeze_v121c_cpo_participation_collapse_close_risk_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_ABC_to_adversarial_review_only_if_external_gap_is_not_trivial",
        }
        interpretation = [
            "V1.21C applies chronology discipline to the first explicit close-risk branch.",
            "A downside line that cannot survive crude year splits is not suitable for promotion even if it looks right on the chart.",
            "This keeps the project from replacing one kind of overfit with another on the risk side.",
        ]
        return V121CCpoParticipationCollapseCloseRiskTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V121CCpoParticipationCollapseCloseRiskTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    result = analyzer.analyze(
        v121b_payload=json.loads((repo_root / "reports" / "analysis" / "v121b_cpo_participation_collapse_close_risk_external_audit_v1.json").read_text(encoding="utf-8")),
    )
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v121c_cpo_participation_collapse_close_risk_time_split_validation_v1",
        result=result,
    )
    print(output_path)
