...
class V118TCpoSustainedParticipationNonChaseDiscoveryReport:
...
class V118TCpoSustainedParticipationNonChaseDiscoveryAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, rows_path: Path) -> V118TCpoSustainedParticipationNonChaseDiscoveryReport:
        rows = [
            dict(row)
            ...
            candidate_score_rows.append(
                {
                    ...
                    "sustained_participation_non_chase_score": round(_candidate_score(row), 6),
                }
            )
        candidate_score_rows.sort(
            key=lambda row: (not bool(row["positive_add_label"]), -_to_float(row["sustained_participation_non_chase_score"]))
        )

        positive_mean = sum(_to_float(row["sustained_participation_non_chase_score"]) for row in candidate_score_rows if row["positive_add_label"]) / len(positive_rows)
        negative_mean = sum(_to_float(row["sustained_participation_non_chase_score"]) for row in candidate_score_rows if not row["positive_add_label"]) / len(negative_rows)

        summary = {
            "acceptance_posture": "freeze_v118t_cpo_sustained_participation_non_chase_discovery_v1",
            ...
            "recommended_next_posture": "audit_sustained_participation_non_chase_on_broader_add_pool_and_keep_candidate_only",
        }
        interpretation = [
            "V1.18T opens a new quality-side line around sustained intraday participation without late-session chase behavior.",
            "The candidate branch rewards persistent occupancy, afternoon participation, and local close-vs-VWAP quality, while penalizing overheated chase-style close location and late-bar extension.",
            "This is discovery only. The branch survives only if the broader external audit stays positive.",
        ]
        return V118TCpoSustainedParticipationNonChaseDiscoveryReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V118TCpoSustainedParticipationNonChaseDiscoveryReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V118TCpoSustainedParticipationNonChaseDiscoveryAnalyzer(repo_root=repo_root)
    result = analyzer.analyze(
        rows_path=repo_root / "data" / "training" / "cpo_midfreq_high_dimensional_feature_base_table_v1.csv",
    )
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v118t_cpo_sustained_participation_non_chase_discovery_v1",
        result=result,
    )
    print(output_path)
