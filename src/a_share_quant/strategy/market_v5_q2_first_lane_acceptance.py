...
class MarketV5Q2FirstLaneAcceptanceReport:
...
class MarketV5Q2FirstLaneAcceptanceAnalyzer:
    """Decide whether the first v5 lane changes the current carry-row-diversity reading."""
    ...
    def analyze(
        ...
        *,
    ...
    ) -> MarketV5Q2FirstLaneAcceptanceReport:
        ...
        acceptance_posture = (
            ...
            if opening_present and not persistence_present and top_driver == "002273"
            else "continue_market_v5_q2_first_lane_structural_check"
        )
        ...
        summary = {
            ...
            "do_open_second_v5_lane_now": False,
        }
        evidence_rows = [
            {
                ...
                "reading": "The first v5 lane should be judged using the strongest q2/B symbol rather than widening to another lane.",
            },
            {
        ...
        interpretation = [
            "The first v5 lane should only upgrade the carry reading if it contributes more than another clean opening case.",
            "If the current 002273 evidence shows a specialist-only opening edge but no clean persistence edge, the lane should remain opening-led rather than being promoted into a carry breakthrough.",
            "So the correct current posture is to treat the first v5 lane as opening-led until a later lane changes the carry-row-diversity reading.",
        ]
        return MarketV5Q2FirstLaneAcceptanceReport(
            summary=summary,
...
def write_market_v5_q2_first_lane_acceptance_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: MarketV5Q2FirstLaneAcceptanceReport,
) -> Path:
