...
class V125WCommercialAerospaceEODSupervisedTrainingPilotReport:
...
class V125WCommercialAerospaceEODSupervisedTrainingPilotAnalyzer:
    ...
    def _fit_centroids(self, train_rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        labels = sorted({row["supervised_action_label"] for row in train_rows})
        centroids: dict[str, dict[str, Any]] = {}
        for label in labels:
            members = [row for row in train_rows if row["supervised_action_label"] == label]
            centroids[label] = {
                "numeric": {
    ...
    def analyze(self) -> V125WCommercialAerospaceEODSupervisedTrainingPilotReport:
        ...
        y_test = [row["supervised_action_label"] for row in test_rows_z]
        ...
        summary = {
            "acceptance_posture": "freeze_v125w_commercial_aerospace_eod_supervised_training_pilot_v1",
            "train_row_count": len(train_rows_z),
            "test_row_count": len(test_rows_z),
            "label_count": len({row['supervised_action_label'] for row in rows}),
            "model_family": "nearest_centroid_eod_candidate",
            "balanced_accuracy": round(_balanced_accuracy(y_test, y_pred), 8),
            "authoritative_status": "first_lawful_eod_supervised_training_pilot_non_execution",
        }
        interpretation = [
            "V1.25W is the first lawful EOD supervised pilot on commercial aerospace after removing the regime-semantic leakage blocker.",
            "It is strictly an offline training sanity check, not a replay rule and not an intraday model.",
        ]
        return V125WCommercialAerospaceEODSupervisedTrainingPilotReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V125WCommercialAerospaceEODSupervisedTrainingPilotReport,
) -> Path:
