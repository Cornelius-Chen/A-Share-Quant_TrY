...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("turnover_discipline_score_candidate")) >= threshold)
    fn = len(positives) - tp
    fp = sum(1 for row in negatives if _to_float(row.get("turnover_discipline_score_candidate")) >= threshold)
    tn = len(negatives) - fp
...
class V119FCpoTushareTurnoverDisciplineTimeSplitValidationReport:
...
class V119FCpoTushareTurnoverDisciplineTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v119e_payload: dict[str, Any]) -> V119FCpoTushareTurnoverDisciplineTimeSplitValidationReport:
        rows = list(v119e_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("turnover_discipline_score_candidate")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v119f_cpo_tushare_turnover_discipline_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_DEF_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.19F applies chronology discipline to the Tushare-backed turnover-discipline branch.",
            "A branch that only looks good after adding new daily data but still fails crude year splits should be stopped before it becomes another data-rich mirage.",
            "This closes the first three-run block for the Tushare orthogonal hardening line.",
        ]
        return V119FCpoTushareTurnoverDisciplineTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V119FCpoTushareTurnoverDisciplineTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V119FCpoTushareTurnoverDisciplineTimeSplitValidationAnalyzer(repo_root=repo_root)
    ...
    output_path = write_report(
        ...
        report_name="v119f_cpo_tushare_turnover_discipline_time_split_validation_v1",
        result=result,
    )
    print(output_path)
