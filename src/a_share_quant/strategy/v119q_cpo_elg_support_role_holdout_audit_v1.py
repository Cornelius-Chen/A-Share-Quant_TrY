...
class V119QCpoElgSupportRoleHoldoutAuditReport:
...
class V119QCpoElgSupportRoleHoldoutAuditAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self) -> V119QCpoElgSupportRoleHoldoutAuditReport:
        rows = _scored_add_rows(self.repo_root)
        holdout_rows: list[dict[str, Any]] = []
        for holdout_role in sorted({str(row["role_family"]) for row in rows}):
            train_rows = [row for row in rows if str(row["role_family"]) != holdout_role]
            test_rows = [row for row in rows if str(row["role_family"]) == holdout_role]
        ...
        summary = {
            "acceptance_posture": "freeze_v119q_cpo_elg_support_role_holdout_audit_v1",
            ...
            "recommended_next_posture": "combine_with_symbol_holdout_and_out_of_set_false_positive_audit_before_reclassification",
        }
        interpretation = [
            "V1.19Q checks whether the ELG-supported hard candidate survives role-family holdouts.",
            "If role holdouts break, hard-candidate language may already be too optimistic.",
            "This remains a non-replay adversarial audit only.",
        ]
        return V119QCpoElgSupportRoleHoldoutAuditReport(summary=summary, holdout_rows=holdout_rows, interpretation=interpretation)


def write_report(*, reports_dir: Path, report_name: str, result: V119QCpoElgSupportRoleHoldoutAuditReport) -> Path:
...
def main() -> None:
    ...
    analyzer = V119QCpoElgSupportRoleHoldoutAuditAnalyzer(repo_root=repo_root)
    result = analyzer.analyze()
    output_path = write_report(reports_dir=repo_root / "reports" / "analysis", report_name="v119q_cpo_elg_support_role_holdout_audit_v1", result=result)
    print(output_path)
