...
@dataclass(slots=True)
class V126PCommercialAerospaceExecutionSurfacePruningAuditReport:
...
class V126PCommercialAerospaceExecutionSurfacePruningAuditAnalyzer:
    def __init__(self, repo_root: Path) -> None:
        self.repo_root = repo_root
        self.daily_path = repo_root / "data" / "raw" / "daily_bars" / "tushare_commercial_aerospace_daily_bars_v1.csv"
        self.reference_path = (
            repo_root / "reports" / "analysis" / "v126o_commercial_aerospace_phase_geometry_walk_forward_shadow_replay_v1.json"
        )
    ...
    def _simulate_variant(self, rows: list[dict[str, Any]], test_dates: list[str], date_to_idx: dict[str, int], config: _VariantConfig) -> dict[str, Any]:
        ordered_dates = sorted({row["trade_date"] for row in rows})
                                ...
                                pending_orders.setdefault(next_trade_date, []).append(
                                    {
                                        "symbol": symbol,
                                        "action": "sell",
                                        "reason": "phase_geometry_risk_low_pass",
                                        "sell_ratio": 0.5,
                                    }
                                )
        ...
        open_rows = [row for row in order_rows if row["action"] == "open"]
        by_date_open_count: dict[str, int] = {}
        by_date_open_symbols: dict[str, set[str]] = {}
        for row in open_rows:
            by_date_open_count[row["execution_trade_date"]] = by_date_open_count.get(row["execution_trade_date"], 0) + 1
            by_date_open_symbols.setdefault(row["execution_trade_date"], set()).add(row["symbol"])
        return {
            "variant": config.name,
            "preheat_cap": config.preheat_cap,
            "impulse_cap": config.impulse_cap,
            "cooldown_days_after_sell": config.cooldown_days_after_sell,
            "min_increment_notional": config.min_increment_notional,
            ...
            "peak_single_day_open_count": max(by_date_open_count.values(), default=0),
            "peak_single_day_open_breadth": max((len(symbols) for symbols in by_date_open_symbols.values()), default=0),
            "tiny_open_order_count_lt_10k": sum(1 for row in open_rows if row["notional"] < 10_000.0),
        }

    def analyze(self) -> V126PCommercialAerospaceExecutionSurfacePruningAuditReport:
        ...
        reference_summary = _load_json(self.reference_path)["summary"]
        variants = [
            _VariantConfig("cap4_cap6_cd3_topup20k", 4, 6, 3, 20_000.0),
            _VariantConfig("cap4_cap6_cd5_topup20k", 4, 6, 5, 20_000.0),
            _VariantConfig("cap3_cap5_cd5_topup20k", 3, 5, 5, 20_000.0),
            _VariantConfig("cap5_cap8_cd3_topup15k", 5, 8, 3, 15_000.0),
        ]
        ...
        summary = {
            "acceptance_posture": "freeze_v126p_commercial_aerospace_execution_surface_pruning_audit_v1",
            "reference_final_equity": reference_summary["final_equity"],
            "reference_max_drawdown": reference_summary["max_drawdown"],
            "reference_executed_order_count": reference_summary["executed_order_count"],
            ...
            "best_variant_peak_single_day_open_breadth": best_variant["peak_single_day_open_breadth"],
            "best_variant_tiny_open_order_count_lt_10k": best_variant["tiny_open_order_count_lt_10k"],
        }
        interpretation = [
            "V1.26P tests whether the main issue in V1.26O is execution-surface width and churn rather than factor or label legality.",
            "Variants only change breadth cap, post-sell cooldown, and minimum top-up size; no factor or label geometry is altered.",
        ]
        return V126PCommercialAerospaceExecutionSurfacePruningAuditReport(summary=summary, variant_rows=variant_rows, interpretation=interpretation)


def write_report(*, reports_dir: Path, report_name: str, result: V126PCommercialAerospaceExecutionSurfacePruningAuditReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v126p_commercial_aerospace_execution_surface_pruning_audit_v1",
        result=result,
    )
    print(output_path)
