...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("cooling_reacceleration_score")) >= threshold)
    fn = len(positives) - tp
    fp = sum(1 for row in negatives if _to_float(row.get("cooling_reacceleration_score")) >= threshold)
    tn = len(negatives) - fp
...
class V117UCpoCoolingReaccelerationTimeSplitValidationReport:
...
class V117UCpoCoolingReaccelerationTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v117t_payload: dict[str, Any]) -> V117UCpoCoolingReaccelerationTimeSplitValidationReport:
        rows = list(v117t_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("cooling_reacceleration_score")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v117u_cpo_cooling_reacceleration_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_STU_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.17U repeats the time-split discipline immediately on the new branch so it cannot hide inside same-sample discovery success.",
            "A branch that cannot survive these crude year splits should be stopped quickly, even if its external-pool full-sample gap looks attractive.",
            "This run exists to save time: it is better to kill a weak branch now than after another replay-facing detour.",
        ]
        return V117UCpoCoolingReaccelerationTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V117UCpoCoolingReaccelerationTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v117u_cpo_cooling_reacceleration_time_split_validation_v1",
        result=result,
    )
    print(output_path)
