...
class MarketV3Q4FirstLaneAcceptanceReport:
...
class MarketV3Q4FirstLaneAcceptanceAnalyzer:
    """Decide whether the first v3 lane changes the current carry reading."""
    ...
    def analyze(
        ...
        *,
    ...
    ) -> MarketV3Q4FirstLaneAcceptanceReport:
        ...
        acceptance_posture = (
            ...
            if opening_present and not persistence_present and top_driver == "002049"
            else "continue_market_v3_q4_first_lane_structural_check"
        )
        ...
        evidence_rows = [
            {
                ...
                "reading": "The first v3 lane should be judged using the strongest q4/C symbol rather than a widened lane set.",
            },
            {
        ...
        interpretation = [
            "The first v3 lane should only upgrade the carry reading if it adds more than another clean opening case.",
            "The current 002049 evidence shows a specialist-only opening edge with aligned permission and entry family, but no clean persistence edge in the checked late window.",
            "So the correct current posture is to treat the first v3 lane as opening-led and keep the second lane closed until a later lane changes the carry-row-diversity reading.",
        ]
        return MarketV3Q4FirstLaneAcceptanceReport(
            summary=summary,
...
def write_market_v3_q4_first_lane_acceptance_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: MarketV3Q4FirstLaneAcceptanceReport,
) -> Path:
