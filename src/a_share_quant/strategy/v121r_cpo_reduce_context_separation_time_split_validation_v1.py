...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("reduce_context_separation_score_candidate")) >= threshold)
    fp = sum(1 for row in negatives if _to_float(row.get("reduce_context_separation_score_candidate")) >= threshold)
    tn = len(negatives) - fp
...
class V121RCpoReduceContextSeparationTimeSplitValidationReport:
...
class V121RCpoReduceContextSeparationTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v121q_payload: dict[str, Any]) -> V121RCpoReduceContextSeparationTimeSplitValidationReport:
        rows = list(v121q_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("reduce_context_separation_score_candidate")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v121r_cpo_reduce_context_separation_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_PQR_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.21R applies chronology discipline to the first explicit reduce-context separation branch.",
            "This is the right chronology test because the branch is trying to separate reduce from other positive action contexts, not just rank generic risk states.",
            "A line that fails here should be downgraded quickly.",
        ]
        return V121RCpoReduceContextSeparationTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V121RCpoReduceContextSeparationTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    result = analyzer.analyze(
        v121q_payload=json.loads((repo_root / "reports" / "analysis" / "v121q_cpo_reduce_context_separation_external_audit_v1.json").read_text(encoding="utf-8")),
    )
    output_path = write_report(
        reports_dir=repo_root / "reports" / "analysis",
        report_name="v121r_cpo_reduce_context_separation_time_split_validation_v1",
        result=result,
    )
    print(output_path)
