...
@dataclass(slots=True)
class V126WCommercialAerospacePreheatRiskConstraintAuditReport:
...
class V126WCommercialAerospacePreheatRiskConstraintAuditAnalyzer:
    ...
    def _simulate_variant(self, rows: list[dict[str, Any]], test_dates: list[str], date_to_idx: dict[str, int], config: _ExecConfig) -> dict[str, Any]:
        ordered_dates = sorted({row["trade_date"] for row in rows})
                        ...
                        for row, es, ds in zip(eval_rows_z, eligibility_eval_scores, de_risk_eval_scores, strict=False):
                            ...
                            if (
                                ...
                                pending_orders.setdefault(next_trade_date, []).append(
                                    {"symbol": symbol, "action": "sell", "reason": "phase_geometry_risk_low_pass", "sell_ratio": config.sell_ratio}
                                )
        ...
        return {
            "variant": config.name,
            "preheat_full_target_notional": config.preheat_full_target_notional,
    ...
    def analyze(self) -> V126WCommercialAerospacePreheatRiskConstraintAuditReport:
        ...
        variants: list[_ExecConfig] = []
        for preheat_notional in (50_000.0, 60_000.0, 70_000.0):
            for sell_ratio in (0.5, 0.75, 1.0):
                variants.append(
                    _ExecConfig(
                        ...
                        preheat_full_target_notional=preheat_notional,
                        sell_ratio=sell_ratio,
                        name=f"pf{int(preheat_notional/1000)}k_sr{int(sell_ratio*100)}",
                    )
                )
        ...
        # Prefer variants that improve DD vs v126v with minimal giveback; tie-break on equity
        best_variant = max(
            variant_rows,
            key=lambda row: (max(v126v["max_drawdown"] - row["max_drawdown"], 0.0), row["final_equity"]),
        )
        summary = {
            "acceptance_posture": "freeze_v126w_commercial_aerospace_preheat_risk_constraint_audit_v1",
        ...
        interpretation = [
            "V1.26W audits whether the downside problem in V1.26V is primarily caused by over-large preheat full sizing or too-weak de-risk sells.",
            "No new factors or labels are introduced; this is pure risk execution tuning on top of the proven preheat-full contribution.",
        ]
        return V126WCommercialAerospacePreheatRiskConstraintAuditReport(summary=summary, variant_rows=variant_rows, interpretation=interpretation)


def write_report(*, reports_dir: Path, report_name: str, result: V126WCommercialAerospacePreheatRiskConstraintAuditReport) -> Path:
