...
TASKS = [
    {
        "task_name": "impulse_eligibility_binary",
        "positive": {"probe_eligibility_target", "full_eligibility_target"},
        "regime_filter": {"impulse_expansion_proxy"},
    },
    {
        "task_name": "riskoff_derisk_binary",
        "positive": {"de_risk_target"},
        "regime_filter": {"risk_off_deterioration_proxy", "sentiment_overdrive_transition_proxy"},
    },
]


@dataclass(slots=True)
class V125ZCommercialAerospaceRegimeConditionedBinaryAuditReport:
...
class V125ZCommercialAerospaceRegimeConditionedBinaryAuditAnalyzer:
    ...
    def analyze(self) -> V125ZCommercialAerospaceRegimeConditionedBinaryAuditReport:
        table = V125TCommercialAerospaceLawfulSupervisedActionTrainingTableAnalyzer(self.repo_root).analyze()
        all_rows = table.training_rows
        task_rows: list[dict[str, Any]] = []

        for task in TASKS:
            rows = [row for row in all_rows if row["regime_proxy_semantic"] in task["regime_filter"]]
            ...
            scores_train = self._score_rows(train_rows_z, train_rows_z, task["positive"])
            scores_test = self._score_rows(train_rows_z, test_rows_z, task["positive"])
            y_train = [1 if row["supervised_action_label"] in task["positive"] else 0 for row in train_rows_z]
            y_test = [1 if row["supervised_action_label"] in task["positive"] else 0 for row in test_rows_z]
            threshold, train_ba = self._best_threshold(scores_train, y_train)
            y_pred = [1 if score >= threshold else 0 for score in scores_test]
            test_ba = _balanced_accuracy_binary(y_test, y_pred)
            threshold_q80 = _quantile(scores_train, 0.8)
            passed = [row for row, score in zip(test_rows_z, scores_test, strict=False) if score >= threshold_q80]
            failed = [row for row, score in zip(test_rows_z, scores_test, strict=False) if score < threshold_q80]
            if "eligibility" in task["task_name"]:
                spread = _mean([float(row["forward_return_10"]) for row in passed]) - _mean([float(row["forward_return_10"]) for row in failed])
            else:
                spread = _mean([float(row["forward_return_10"]) for row in failed]) - _mean([float(row["forward_return_10"]) for row in passed])
            task_rows.append(
                {
                    "task_name": task["task_name"],
                    "row_count": len(rows),
                    ...
                    "q80_test_spread": round(spread, 8),
                    "q80_pass_count": len(passed),
                    "status": "ok",
                }
            )

        valid_rows = [row for row in task_rows if row["status"] == "ok"]
        best_task = max(valid_rows, key=lambda row: row["test_balanced_accuracy"]) if valid_rows else {"task_name": "none", "test_balanced_accuracy": 0.0}
        summary = {
            "acceptance_posture": "freeze_v125z_commercial_aerospace_regime_conditioned_binary_audit_v1",
            "task_count": len(task_rows),
            "best_task_name": best_task["task_name"],
            "best_test_balanced_accuracy": best_task["test_balanced_accuracy"],
            "authoritative_status": "lawful_eod_regime_conditioned_binary_audit_non_execution",
        }
        interpretation = [
            "V1.25Z asks the more realistic question: can lawful EOD supervision separate eligibility and de-risk better once the board is already inside the relevant regime proxy?",
            "This is the last audit step before considering any replay-adjacent shadow protocol.",
        ]
        return V125ZCommercialAerospaceRegimeConditionedBinaryAuditReport(
            summary=summary,
            task_rows=task_rows,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V125ZCommercialAerospaceRegimeConditionedBinaryAuditReport,
) -> Path:
