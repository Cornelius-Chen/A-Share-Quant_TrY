...
class V125TCommercialAerospaceLawfulSupervisedActionTrainingTableReport:
...
class V125TCommercialAerospaceLawfulSupervisedActionTrainingTableAnalyzer:
    ...
    def analyze(self) -> V125TCommercialAerospaceLawfulSupervisedActionTrainingTableReport:
        ...
        regime_proxy = V125SCommercialAerospacePointInTimeRegimeProxyAnalyzer(self.repo_root).analyze()
        regime_map = {row["trade_date"]: row["regime_proxy_semantic"] for row in regime_proxy.regime_rows}
        event_maps = V125NCommercialAerospaceStructuralRegimeDiscoveryAnalyzer(self.repo_root)._event_maps()
        ...
        for row in row_pool:
            ...
            if key not in future_extremes or row["trade_date"] not in regime_map:
                continue
            enriched = {
                "trade_date": row["trade_date"],
                "symbol": row["symbol"],
                "role_layer": row["layer"],
                "regime_proxy_semantic": regime_map[row["trade_date"]],
        ...
        summary = {
            "acceptance_posture": "freeze_v125t_commercial_aerospace_lawful_supervised_action_training_table_v1",
            "row_count": len(base_rows),
            "symbol_count": len({row["symbol"] for row in base_rows}),
            "feature_count": 18,
            "point_in_time_regime_proxy": True,
            ...
            "authoritative_rule": "lawful_supervised_learning_uses_point_in_time_regime_proxy_plus_decisive_event_state_and_keeps_future_path_quantities_label_only",
        }
        interpretation = [
            "V1.25T rebuilds the commercial-aerospace supervised table with a point-in-time regime proxy instead of the full-sample structural regime label.",
            "This is the lawful EOD supervision table for later time-series training and audit.",
        ]
        return V125TCommercialAerospaceLawfulSupervisedActionTrainingTableReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V125TCommercialAerospaceLawfulSupervisedActionTrainingTableReport,
) -> Path:
