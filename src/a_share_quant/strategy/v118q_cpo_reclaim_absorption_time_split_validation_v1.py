...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("reclaim_absorption_score")) >= threshold)
    fn = len(positives) - tp
    fp = sum(1 for row in negatives if _to_float(row.get("reclaim_absorption_score")) >= threshold)
    tn = len(negatives) - fp
...
class V118QCpoReclaimAbsorptionTimeSplitValidationReport:
...
class V118QCpoReclaimAbsorptionTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v118p_payload: dict[str, Any]) -> V118QCpoReclaimAbsorptionTimeSplitValidationReport:
        rows = list(v118p_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("reclaim_absorption_score")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v118q_cpo_reclaim_absorption_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_OPQ_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.18Q applies the same immediate chronology discipline to the reclaim-absorption branch.",
            "A branch that cannot survive crude year splits should not absorb more research budget, even if the full-sample gap looks attractive.",
            "This keeps discovery honest and prevents semantic restatement from masquerading as robust action logic.",
        ]
        return V118QCpoReclaimAbsorptionTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V118QCpoReclaimAbsorptionTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V118QCpoReclaimAbsorptionTimeSplitValidationAnalyzer(repo_root=repo_root)
    ...
    output_path = write_report(
        ...
        report_name="v118q_cpo_reclaim_absorption_time_split_validation_v1",
        result=result,
    )
    print(output_path)
