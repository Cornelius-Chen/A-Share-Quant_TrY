...
@dataclass(slots=True)
class V127ECommercialAerospaceOrthogonalDownsideExecutionScanReport:
...
class V127ECommercialAerospaceOrthogonalDownsideExecutionScanAnalyzer:
    ...
    def _family_match(self, candidate: dict[str, Any], family: str) -> bool:
        if family == "broad_half_reference":
            return True
        if family == "overdrive_continuation_half":
            return (
                candidate["regime_proxy_semantic"] == "sentiment_overdrive_transition_proxy"
                and candidate["event_state"] == "continuation_active"
            )
        return (
    ...
    def analyze(self) -> V127ECommercialAerospaceOrthogonalDownsideExecutionScanReport:
        ...
        variants = [
            ...
            _Config("overdrive_continuation_half", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "overdrive_continuation_half", 0.5),
            _Config("weak_drift_event_neutral_half", int(base["preheat_cap"]), int(base["impulse_cap"]), int(base["cooldown_days_after_sell"]), float(base["min_increment_notional"]), 70_000.0, "weak_drift_event_neutral_half", 0.5),
        ]
        ...
        summary = {
            "acceptance_posture": "freeze_v127e_commercial_aerospace_orthogonal_downside_execution_scan_v1",
        ...
        interpretation = [
            "V1.27E scans a genuinely orthogonal downside family rather than narrowing the same phase-aware branch again.",
            "The candidate families come from the weakest semantic slices in the recent label table: overdrive+continuation and weak-drift+event-neutral.",
        ]
        return V127ECommercialAerospaceOrthogonalDownsideExecutionScanReport(
            summary=summary,
...
def write_report(*, reports_dir: Path, report_name: str, result: V127ECommercialAerospaceOrthogonalDownsideExecutionScanReport) -> Path:
...
def main() -> None:
    ...
    output_path = write_report(
        ...
        report_name="v127e_commercial_aerospace_orthogonal_downside_execution_scan_v1",
        result=result,
    )
    print(output_path)
