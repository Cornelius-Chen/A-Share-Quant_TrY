...
class V112BFCPOAggressiveNoLeakBlackBoxPortfolioPilotReport:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "oracle_gap_rows": self.oracle_gap_rows,
...
class V112BFCPOAggressiveNoLeakBlackBoxPortfolioPilotAnalyzer:
    HOLDING_DAYS = 20
    ...
    def analyze(
        ...
        *,
    ...
    ) -> V112BFCPOAggressiveNoLeakBlackBoxPortfolioPilotReport:
        if not bool(dict(phase_charter_payload.get("summary", {})).get("do_open_v112bf_now")):
            raise ValueError("V1.12BF must be open before the aggressive no-leak pilot runs.")
        if int(dict(v112bc_protocol_payload.get("summary", {})).get("no_leak_track_count", 0)) != 2:
            raise ValueError("V1.12BF requires the frozen no-leak tracks from V1.12BC.")

        oracle_summary = dict(oracle_benchmark_payload.get("summary", {}))
        oracle_trade_count = int(oracle_summary.get("trade_count", 0))
        ...
        if len(training_layer_rows) != 10:
            raise ValueError("V1.12BF expects the frozen 10-row training-facing layer.")
        ...
        while idx < len(ordered_dates):
            ...
            chosen_row = current_frame.sort_values(["winner_prob", "forward_return_20d"], ascending=[False, False]).iloc[0]
            if float(chosen_row["winner_prob"]) <= base_rate:
                self._append_equity_point(equity_curve_rows, trade_date, current_equity, "cash", "CASH")
                idx += 1
                continue
            ...
            trade_rows.append(
                {
                    ...
                    "training_base_rate": round(base_rate, 4),
        ...
        oracle_gap_rows = [
            {
                "gap_name": "total_return_gap",
                "oracle_value": oracle_total_return,
                "aggressive_no_leak_value": round(total_return, 4),
                "gap": round(oracle_total_return - total_return, 4),
            },
            {
                "gap_name": "trade_count_gap",
                "oracle_value": oracle_trade_count,
                "aggressive_no_leak_value": len(trade_rows),
                "gap": oracle_trade_count - len(trade_rows),
            },
            {
                "gap_name": "max_drawdown_gap",
                "oracle_value": oracle_summary.get("max_drawdown"),
                "aggressive_no_leak_value": round(float(max_drawdown), 4),
                "gap": round(float(oracle_summary.get("max_drawdown", 0.0)) - float(max_drawdown), 4),
            },
        ]

        summary = {
            "acceptance_posture": "freeze_v112bf_cpo_aggressive_no_leak_black_box_portfolio_pilot_v1",
            ...
            "oracle_return_gap": round(oracle_total_return - total_return, 4),
            "formal_training_now": False,
            "formal_signal_generation_now": False,
            "ready_for_phase_check_next": True,
            "recommended_next_posture": "review_oracle_vs_no_leak_gap_and_then_open_neutral_selective_track",
        }
        interpretation = [
            "This pilot uses only point-in-time visible information by training on samples whose 20-day future window has fully elapsed before each decision date.",
            "The aggressive track prioritizes return and uses a winner-probability ranking to select at most one position or stay in cash.",
            "This remains a bounded experimental portfolio line and does not open formal signal rights.",
        ]
        return V112BFCPOAggressiveNoLeakBlackBoxPortfolioPilotReport(
            summary=summary,
            ...
            plot_bundle_rows=[
                {"plot_name": "aggressive_no_leak_equity_curve", "series_name": "equity", "point_count": len(equity_curve_rows)},
                {"plot_name": "aggressive_no_leak_drawdown_curve", "series_name": "drawdown", "point_count": len(drawdown_curve_rows)},
            ],
...
def write_v112bf_cpo_aggressive_no_leak_black_box_portfolio_pilot_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V112BFCPOAggressiveNoLeakBlackBoxPortfolioPilotReport,
) -> Path:
