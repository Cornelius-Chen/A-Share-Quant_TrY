...
def _balanced_accuracy(rows: list[dict[str, Any]], threshold: float) -> dict[str, float]:
    ...
    tp = sum(1 for row in positives if _to_float(row.get("participation_turnover_combo_score")) >= threshold)
    fn = len(positives) - tp
    fp = sum(1 for row in negatives if _to_float(row.get("participation_turnover_combo_score")) >= threshold)
    tn = len(negatives) - fp
...
class V119JCpoParticipationTurnoverComboTimeSplitValidationReport:
...
class V119JCpoParticipationTurnoverComboTimeSplitValidationAnalyzer:
    def __init__(self, *, repo_root: Path) -> None:
        self.repo_root = repo_root

    def analyze(self, *, v119i_payload: dict[str, Any]) -> V119JCpoParticipationTurnoverComboTimeSplitValidationReport:
        rows = list(v119i_payload.get("scored_rows", []))
        split_defs = (
            ("holdout_2023", {"2023"}),
            ("holdout_2024", {"2024"}),
        ...
        for split_name, holdout_years in split_defs:
            ...
            thresholds = sorted({_to_float(row.get("participation_turnover_combo_score")) for row in train_rows}, reverse=True)
            best_train = None
        ...
        summary = {
            "acceptance_posture": "freeze_v119j_cpo_participation_turnover_combo_time_split_validation_v1",
            ...
            "recommended_next_posture": "send_HIJ_to_three_run_adversarial_review",
        }
        interpretation = [
            "V1.19J checks whether the minimal combo branch really repairs the 2024 chronology hole rather than just making the full sample look nicer.",
            "A combined branch that still fails crude year splits should be dropped immediately instead of being treated as clever stacking.",
            "This closes the first three-run block for the participation-turnover combo line.",
        ]
        return V119JCpoParticipationTurnoverComboTimeSplitValidationReport(
            summary=summary,
...
def write_report(
    *,
    reports_dir: Path,
    report_name: str,
    result: V119JCpoParticipationTurnoverComboTimeSplitValidationReport,
) -> Path:
...
def main() -> None:
    ...
    analyzer = V119JCpoParticipationTurnoverComboTimeSplitValidationAnalyzer(repo_root=repo_root)
    ...
    output_path = write_report(
        ...
        report_name="v119j_cpo_participation_turnover_combo_time_split_validation_v1",
        result=result,
    )
    print(output_path)
