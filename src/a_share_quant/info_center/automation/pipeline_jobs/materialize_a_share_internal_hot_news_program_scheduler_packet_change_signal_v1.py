...
class MaterializedAShareInternalHotNewsProgramSchedulerPacketChangeSignalV1:
    summary: dict[str, Any]
    rows: list[dict[str, Any]]


class MaterializeAShareInternalHotNewsProgramSchedulerPacketChangeSignalV1:
    def __init__(self, repo_root: Path) -> None:
        ...
        self.history_path = (
            ...
            / "info_center"
            / "serving_registry"
            / "a_share_internal_hot_news_program_scheduler_packet_history_v1.csv"
        )
        self.output_path = (
            repo_root
            / "data"
        ...
        self.registry_path = (
            ...
            / "info_center"
            / "serving_registry"
            / "a_share_internal_hot_news_program_scheduler_packet_change_signal_registry_v1.csv"
        )
    ...
    def materialize(self) -> MaterializedAShareInternalHotNewsProgramSchedulerPacketChangeSignalV1:
        current = _read_csv(self.packet_path)[0]
        previous_rows = _read_csv(self.history_path)
        previous = previous_rows[0] if previous_rows else None

        scheduler_mode_change = _state_change(
            current["scheduler_mode"],
            previous["scheduler_mode"] if previous else None,
        )
        ...
        row = {
            "scheduler_packet_change_signal_id": "internal_hot_news_program_scheduler_packet_change_latest",
            "scheduler_mode_change": scheduler_mode_change,
            ...
            "delivery_state": "program_scheduler_packet_change_signal_ready",
        }
        ...
        self._write_csv(
            ...
            [
                {
                    "view_id": "internal_hot_news_program_scheduler_packet_change_signal",
        ...
        summary = {
            ...
            "authoritative_output": "a_share_internal_hot_news_program_scheduler_packet_change_signal_materialized",
        }
        return MaterializedAShareInternalHotNewsProgramSchedulerPacketChangeSignalV1(summary=summary, rows=[row])


def main() -> None:
    repo_root = Path(__file__).resolve().parents[4]
    result = MaterializeAShareInternalHotNewsProgramSchedulerPacketChangeSignalV1(repo_root).materialize()
