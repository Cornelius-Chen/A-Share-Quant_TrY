...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare segmentation methods for one strategy family.")
    parser.add_argument(
        "--config",
        default="config/demo_segmentation_comparison.yaml",
        help="Path to the segmentation comparison YAML config.",
    )
...
def main() -> None:
    ...
    comparison_runner = SegmentationComparisonRunner(engine=engine, config=config)
    ...
    comparison = comparison_runner.run(
    ...
    run_record = registry.create_run(
        ...
        run_type="segmentation_comparison",
        strategy_family=strategy_name,
        notes="Compare segmentation methods under one shared strategy and config.",
    )
    report_path = write_comparison_report(
        ...
        extras={
            "strategy_name": strategy_name,
            "segmentation_methods": segmentation_methods,
            "comparison_basis": "shared_strategy_shared_data_different_segmentation_method",
        },
    )
    ...
    for row in rows:
        print(f"{row['segmentation_method']}: {row['summary']}")
