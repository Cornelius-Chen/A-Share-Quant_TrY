...
from a_share_quant.strategy.feature_pack_c_late_quality_residuals import (
    ...
    load_json_report,
    write_feature_pack_c_late_quality_residuals_report,
)


def main() -> None:
    config = load_yaml_config(Path("config/feature_pack_c_late_quality_residuals_v1.yaml"))
    result = FeaturePackCLateQualityResidualsAnalyzer().analyze(
        recheck_payload=load_json_report(Path(str(config["inputs"]["recheck_report"]))),
        stock_snapshots_csv=Path(str(config["inputs"]["stock_snapshots_csv"])),
        case_names=[str(item) for item in config["inputs"]["case_names"]],
    )
    output_path = write_feature_pack_c_late_quality_residuals_report(
