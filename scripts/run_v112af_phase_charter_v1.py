...
from a_share_quant.strategy.v112af_phase_charter_v1 import (
    ...
    load_json_report,
    write_v112af_phase_charter_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12AF phase charter.")
...
def main() -> None:
    ...
    output_path = write_v112af_phase_charter_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12AF phase charter report: {output_path}")
