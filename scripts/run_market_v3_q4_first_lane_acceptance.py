...
from a_share_quant.strategy.market_v3_q4_first_lane_acceptance import (
    ...
    load_json_report,
    write_market_v3_q4_first_lane_acceptance_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Decide whether the first v3 q4 lane changes the current carry reading."
    )
    parser.add_argument(
        "--config",
        default="config/market_v3_q4_first_lane_acceptance_v1.yaml",
        help="Path to the v3 q4 first-lane acceptance YAML config.",
    )
...
def main() -> None:
    ...
    output_path = write_market_v3_q4_first_lane_acceptance_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"Market-v3 q4 first-lane acceptance report: {output_path}")
