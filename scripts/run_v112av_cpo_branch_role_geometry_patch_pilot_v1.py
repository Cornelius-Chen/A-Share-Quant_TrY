...
from a_share_quant.strategy.v112av_cpo_branch_role_geometry_patch_pilot_v1 import (
    ...
    load_json_report,
    write_v112av_cpo_branch_role_geometry_patch_pilot_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12AV branch role geometry patch pilot.")
...
def main() -> None:
    ...
    result = analyzer.analyze(
        ...
        widen_pilot_payload=load_json_report(Path(config["paths"]["v112au_widen_report"])),
    ...
    output_path = write_v112av_cpo_branch_role_geometry_patch_pilot_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12AV branch role geometry patch pilot report: {output_path}")
