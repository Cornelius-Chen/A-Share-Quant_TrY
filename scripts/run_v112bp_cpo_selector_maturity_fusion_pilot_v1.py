...
from a_share_quant.strategy.v112bp_cpo_selector_maturity_fusion_pilot_v1 import (
    ...
    load_json_report,
    write_v112bp_cpo_selector_maturity_fusion_pilot_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12BP selector maturity fusion pilot.")
...
def main() -> None:
    ...
    result = analyzer.analyze(
        ...
        ranker_search_payload=load_json_report(Path(config["paths"]["ranker_search_report"])),
    ...
    output_path = write_v112bp_cpo_selector_maturity_fusion_pilot_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12BP fusion pilot report: {output_path}")
