...
from a_share_quant.strategy.late_mover_admissibility import (
    ...
    write_late_mover_admissibility_report,
)
from a_share_quant.strategy.rule_sweep import RuleCandidate


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze theme late-mover admissibility gaps.")
    parser.add_argument(
        "--config",
        default="config/theme_late_mover_admissibility_v1.yaml",
        help="Path to the late-mover admissibility YAML config.",
    )
...
def main() -> None:
    ...
    result = LateMoverAdmissibilityAnalyzer().analyze(
        ...
        top_k=int(config["analysis"].get("top_k", 20)),
        engine=engine,
    )
    report_path = write_late_mover_admissibility_report(
    ...
    print(f"Late-mover admissibility report: {report_path}")
