...
from a_share_quant.strategy.v112bi_phase_check_v1 import (
    ...
    load_json_report,
    write_v112bi_phase_check_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12BI phase check.")
...
def main() -> None:
    ...
    output_path = write_v112bi_phase_check_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12BI phase check report: {output_path}")
