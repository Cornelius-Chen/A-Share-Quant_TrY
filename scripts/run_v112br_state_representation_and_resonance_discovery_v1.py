...
from a_share_quant.strategy.v112br_state_representation_and_resonance_discovery_v1 import (
    ...
    load_json_report,
    write_v112br_state_representation_and_resonance_discovery_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12BR state representation and resonance discovery.")
...
def main() -> None:
    ...
    output_path = write_v112br_state_representation_and_resonance_discovery_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12BR resonance discovery report: {output_path}")
