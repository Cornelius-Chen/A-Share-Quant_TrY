...
from a_share_quant.strategy.feature_pack_c_turnover_share_context import (
    ...
    load_json_report,
    write_feature_pack_c_turnover_share_context_report,
)


def main() -> None:
    config = load_yaml_config(Path("config/feature_pack_c_turnover_share_context_v1.yaml"))
    result = FeaturePackCTurnoverShareContextAnalyzer().analyze(
        context_payload=load_json_report(Path(str(config["inputs"]["context_report"]))),
        stock_snapshots_csv=Path(str(config["inputs"]["stock_snapshots_csv"])),
        case_names=[str(item) for item in config["inputs"]["case_names"]],
    )
    output_path = write_feature_pack_c_turnover_share_context_report(
