...
from a_share_quant.strategy.v112bk_cpo_tree_ranker_search_v1 import (
    ...
    load_json_report,
    write_v112bk_cpo_tree_ranker_search_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V1.12BK CPO tree/ranker search.")
...
def main() -> None:
    ...
    output_path = write_v112bk_cpo_tree_ranker_search_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.12BK tree/ranker search report: {output_path}")
