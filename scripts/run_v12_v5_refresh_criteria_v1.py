...
from a_share_quant.strategy.v12_v5_refresh_criteria_v1 import (
    ...
    load_json_report,
    write_v12_v5_refresh_criteria_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Freeze symbol-selection criteria for the V1.2 v5 refresh.")
    parser.add_argument(
        "--config",
        default="config/v12_v5_refresh_criteria_v1.yaml",
        help="Path to the v12 v5 refresh criteria YAML config.",
    )
...
def main() -> None:
    ...
    output_path = write_v12_v5_refresh_criteria_report(
        reports_dir=Path(config["paths"]["reports_dir"]),
        report_name=str(config["report"]["name"]),
        result=result,
    )
    print(f"V1.2 v5 refresh criteria: {output_path}")
