...
def test_merge_config_recursively_overrides_nested_values() -> None:
    base = {
        "regime": {"min_top_score": 2.5, "min_score_margin": 0.0},
        "trend": {"filters": {"short_window": 2, "medium_window": 3}},
    }
    override = {
        "regime": {"min_top_score": 3.0},
        "trend": {"filters": {"medium_window": 4}},
    }

    merged = merge_config(base, override)

    assert merged["regime"]["min_top_score"] == 3.0
    assert merged["regime"]["min_score_margin"] == 0.0
    assert merged["trend"]["filters"]["short_window"] == 2
    assert merged["trend"]["filters"]["medium_window"] == 4
    assert base["regime"]["min_top_score"] == 2.5
...
def test_rule_sweep_runner_emits_rows_and_candidate_leaderboard() -> None:
    ...
    candidates = [
        RuleCandidate(candidate_name="control", description="baseline", override={}),
        RuleCandidate(
            candidate_name="strict",
            description="stricter thresholds",
            override={"regime": {"min_top_score": 3.0}},
        ),
    ]
    ...
    assert leaderboard[0]["composite_rank_score"] <= leaderboard[1]["composite_rank_score"]
    assert summary["comparison_count"] == 8
    assert summary["candidate_count"] == 2
    assert "candidate_name" in summary["most_stable_candidate"]
