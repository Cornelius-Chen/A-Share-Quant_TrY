...
def test_v134ug_runtime_stack_supervisor_loop_signal_change_summary() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    report = V134UGAShareInternalHotNewsProgramRuntimeStackSupervisorLoopSignalChangeAuditV1Analyzer(repo_root).analyze()

    assert report.summary["signal_row_count"] == 1
    assert report.summary["signal_priority"] in {"p1", "p2"}
    assert report.summary["supervision_loop_mode_change"] in {"no_previous_baseline", "state_changed", "stable"}
    assert report.summary["reopen_target_change"] in {"no_previous_baseline", "state_changed", "stable"}


def test_v134ug_runtime_stack_supervisor_loop_signal_change_rows() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    report = V134UGAShareInternalHotNewsProgramRuntimeStackSupervisorLoopSignalChangeAuditV1Analyzer(repo_root).analyze()
    rows = {row["component"]: row["component_state"] for row in report.rows}

    assert rows["runtime_stack_supervisor_loop_signal_change"] == "read_ready_internal_only"
