...
def test_v134uo_runtime_stack_control_packet_change_summary() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    report = V134UOAShareInternalHotNewsProgramRuntimeStackControlPacketChangeSignalAuditV1Analyzer(repo_root).analyze()
...
def test_v134uo_runtime_stack_control_packet_change_rows() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    report = V134UOAShareInternalHotNewsProgramRuntimeStackControlPacketChangeSignalAuditV1Analyzer(repo_root).analyze()
    rows = {row["component"]: row["component_state"] for row in report.rows}

    assert rows["runtime_stack_control_packet_change_signal"] == "read_ready_internal_only"
