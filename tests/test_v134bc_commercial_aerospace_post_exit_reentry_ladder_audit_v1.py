from __future__ import annotations

import json
import subprocess
from pathlib import Path


def test_v134bc_commercial_aerospace_post_exit_reentry_ladder_audit_v1() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    subprocess.run(
        ["python", "scripts/run_v134ba_commercial_aerospace_post_exit_reentry_timing_supervision_audit_v1.py"],
        cwd=repo_root,
        check=True,
    )
    subprocess.run(
        ["python", "scripts/run_v134bc_commercial_aerospace_post_exit_reentry_ladder_audit_v1.py"],
        cwd=repo_root,
        check=True,
    )
    report_path = (
        repo_root / "reports" / "analysis" / "v134bc_commercial_aerospace_post_exit_reentry_ladder_audit_v1.json"
    )
    payload = json.loads(report_path.read_text(encoding="utf-8"))
    assert payload["summary"]["seed_count"] == 3
    assert payload["summary"]["same_day_entry_authorized_seed_count"] == 0
    assert payload["summary"]["persistent_recovery_seed_count"] == 2
    assert payload["summary"]["late_only_recovery_seed_count"] == 1
