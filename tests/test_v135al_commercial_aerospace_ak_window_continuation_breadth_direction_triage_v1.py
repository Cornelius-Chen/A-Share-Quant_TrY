from __future__ import annotations

from pathlib import Path

from a_share_quant.strategy.v135al_commercial_aerospace_ak_window_continuation_breadth_direction_triage_v1 import (
    V135ALCommercialAerospaceAKWindowContinuationBreadthDirectionTriageV1Analyzer,
)


def test_v135al_continuation_breadth_direction_triage() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    result = V135ALCommercialAerospaceAKWindowContinuationBreadthDirectionTriageV1Analyzer(repo_root).analyze()
    assert result.summary["window_count"] == 1
    assert result.summary["full_window_hold_count"] == 1
    assert result.triage_rows[0]["sample_window_id"] == "ca_train_window_008"
    assert "keep_subwindow_learning_only" in result.triage_rows[0]["recommendation"]
