...
def test_strategy_suite_runner_returns_comparable_results() -> None:
    bars = []
    ...
    assert [item.strategy_name for item in suite.results] == [
        "mainline_trend_a",
        "mainline_trend_b",
        "mainline_trend_c",
    ]
    assert [item.backtest_result.summary["signal_count"] for item in suite.results] == [2, 4, 6]
    rows = suite.comparison_rows()
    assert rows[0]["ranks"]["total_return"] >= 1
    assert rows[0]["ranks"]["mainline_capture_ratio"] >= 1
    assert rows[0]["ranks"]["max_drawdown"] >= 1
    assert "trade_overview" in rows[0]
    assert "window_breakdown" in rows[0]
